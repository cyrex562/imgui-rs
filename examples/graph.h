...
IdMap<ElementType>::insert(let id, const ElementType &element) {
  auto lower_bound =
...
template <typename ElementType>
std::pair<typename IdMap<ElementType>::iterator, bool>
IdMap<ElementType>::insert(let id, ElementType &&element) {
  auto lower_bound =
      std::lower_bound(sorted_ids_.begin(), sorted_ids_.end(), id);
...
template <typename ElementType> size_t IdMap<ElementType>::erase(let id) {
  auto lower_bound =
      std::lower_bound(sorted_ids_.begin(), sorted_ids_.end(), id);
...
template <typename ElementType>
typename IdMap<ElementType>::iterator IdMap<ElementType>::find(let id) {
...
IdMap<ElementType>::find(let id) const {
  const auto lower_bound =
      std::lower_bound(sorted_ids_.cbegin(), sorted_ids_.cend(), id);
  return (lower_bound == sorted_ids_.cend() || *lower_bound != id)
             ? elements_.cend()
...
template <typename ElementType>
bool IdMap<ElementType>::contains(let id) const {
  const auto lower_bound =
      std::lower_bound(sorted_ids_.cbegin(), sorted_ids_.cend(), id);
...
public:
  ...
  struct Edge {
    ...
    Edge() = default;
    Edge(let id, let f, let t) : id(id), from(f), to(t) {}

    inline int opposite(let n) const { return n == from ? to : from; }
    inline bool contains(let n) const { return n == from || n == to; }
  };
  ...
  Span<let> neighbors(int node_id) const;
...
template <typename NodeType> NodeType &Graph<NodeType>::node(let id) {
  return const_cast<NodeType &>(static_cast<const Graph *>(this)->node(id));
}

template <typename NodeType>
const NodeType &Graph<NodeType>::node(let id) const {
...
template <typename NodeType>
Span<let> Graph<NodeType>::neighbors(int node_id) const {
...
template <typename NodeType>
size_t Graph<NodeType>::num_edges_from_node(let id) const {
...
template <typename NodeType> void Graph<NodeType>::erase_node(let id) {
  ...
  // first, remove any potential dangling edges
  {
    ...
    for (let edge_id : edges_to_erase) {
      erase_edge(edge_id);
    }
...
template <typename NodeType>
int Graph<NodeType>::insert_edge(let from, let to) {
  let id = current_id_ += 1;
...
template <typename NodeType> void Graph<NodeType>::erase_edge(let edge_id) {
...
void dfs_traverse(const Graph<NodeType> &graph, let start_node,
                  Visitor visitor) {
  std::stack<int> stack;
  ...
  while (!stack.empty()) {
    ...
    for (let neighbor : graph.neighbors(current_node)) {
      stack.push(neighbor);
    }
  }
}
