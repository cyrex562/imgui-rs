...
int main(int, char**)
{
    ...
    SDL_Vulkan_GetInstanceExtensions(window, &extensions_count, None);
    ...
    //io.config_flags |= ImGuiConfigFlags_ViewportsNoTaskBarIcons;
    //io.config_flags |= ImGuiConfigFlags_ViewportsNoMerge;
    ...
    while (!done)
    {
        ...
        const bool main_is_minimized = (main_draw_data->DisplaySize.x <= 0.0 || main_draw_data->DisplaySize.y <= 0.0);
