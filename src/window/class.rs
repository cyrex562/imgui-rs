...
pub struct WindowClass
{
    ...
    pub viewport_flags_override_set: HashSet<ViewportFlags>,   // viewport flags to set when a window of this class owns a viewport. This allows you to enforce OS decoration or task bar icon, override the defaults on a per-window basis.
    pub viewport_flags_override_clear: HashSet<ViewportFlags>, // viewport flags to clear when a window of this class owns a viewport. This allows you to enforce OS decoration or task bar icon, override the defaults on a per-window basis.
    pub tab_item_flags_override_set: HashSet<TabItemFlags>,    // [EXPERIMENTAL] TabItem flags to set when a window of this class gets submitted into a dock node tab bar. May use with ImGuiTabItemFlags_Leading or ImGuiTabItemFlags_Trailing.
    pub dock_node_flags_override_set: HashSet<DockNodeFlags>,   // [EXPERIMENTAL] Dock node flags to set when a window of this class is hosted by a dock node (it doesn't have to be selected!)
