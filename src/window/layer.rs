...
// void ImGui::BringWindowToFocusFront(Window* window)
...
// void ImGui::bring_window_to_display_front(Window* window)
pub fn bring_window_to_display_front(g: &mut Context, window: &mut Window)
{
    ...
    for (int i = g.windows.len() - 2; i >= 0; i -= 1 ) // We can ignore the top-most window
        ...
        {
            memmove(&g.windows[i], &g.windows[i + 1], (g.windows.len() - i - 1) * sizeof(Window*));
            g.windows[g.windows.len() - 1] = window;
            break;
        }
}

// void ImGui::BringWindowToDisplayBack(Window* window)
pub fn bring_window_to_display_back(g: &mut Context, window: &mut Window)
{
    ...
    for (int i = 0; i < g.windows.len(); i += 1)
        if (g.windows[i] == window)
        {
            memmove(&g.windows[1], &g.windows[0], i * sizeof(Window*));
            g.windows[0] = window;
            break;
        }
}

// void ImGui::BringWindowToDisplayBehind(Window* window, Window* behind_window)
...
// void ImGui::focus_window(Window* window)
pub fn focus_window(g: &mut Context, window: &mut Window)
{
    ...
    Window* focus_front_window = window ? window.root_window : None;
    Window* display_front_window = window ? window.root_window_dock_tree : None;
...
// void ImGui::focus_topmost_window_under_one(Window* under_this_window, Window* ignore_window)
pub fn focus_top_most_window_under_one(g: &mut Context, window: &mut under_this_window, ignore_window: &mut Window)
{
    ...
    for (int i = start_idx; i >= 0; i -= 1 )
    {
        ...
        Window* window = g.windows_focus_order[i];
        // IM_ASSERT(window == window.root_window);
