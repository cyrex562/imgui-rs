    ...
    else
    {
        ...
        if flags.contains(&WindowFlags::MenuBar)
        {
            ...
            if style.frame_border_size > 0.0 && menu_bar_rect.max.y < window.pos.y + window.size.y {
                let x = window.draw_list.add_line(menu_bar_rect.get_bl(), menu_bar_rect.GetBR(), get_color_u32_no_alpha(StyleColor::Border), style.frame_border_size);
            }
        }
...
pub fn render_window_title_bar_contents(g: &mut Context, window: &mut Window, title_bar_rect: &Rect, name: &str, p_open: Option<&mut bool>)
{
    // ImGuiContext& g = *GImGui;
    // ImGuiStyle& style = g.style;
    ...
    let has_collapse_button = flags.contains(&WindowFlags::NoCollapse) == false && style.window_menu_button_position != Direction::None;
    ...
    if has_collapse_button {
        if collapse_button(window.get_id(g, "#COLLAPSE"), collapse_button_pos, NULL) {
            window.WantCollapseToggle = true;
        }// Defer actual collapsing to next frame as we are too far in the Begin() function
    }
    // Close button
    if has_close_button {
        if close_button(window.get_id(g, "#CLOSE"), close_button_pos) { *p_open = false; }
    }
    ...
    let marker_size_x = if flags.contains(&WindowFlags::UnsavedDocument) { button_sz * 0.80} else { 0.0 };
    ...
    if style.window_title_align.x > 0.0 && style.window_title_align.x < 1.0
    {
        ...
        let pad_extend = f32::min(f32::max(pad_l, pad_r), title_bar_rect.get_width() - pad_l - pad_r - text_size.x);
        pad_l = f32::max(pad_l, pad_extend * centerness);
        pad_r = f32::max(pad_r, pad_extend * centerness);
    }

    let  layout_r = Rect::new4(title_bar_rect.min.x + pad_l, title_bar_rect.min.y, title_bar_rect.max.x - pad_r, title_bar_rect.max.y);
    let mut clip_r = Rect::new4(layout_r.min.x, layout_r.min.y, f32::min(layout_r.max.x + g.style.item_inner_spacing.x, title_bar_rect.max.x), layout_r.max.y);
    if flags.contains(&WindowFlags::UnsavedDocument)
    {
        // Vector2D marker_pos;
        let mut marker_pos = Vector2D::default();
        marker_pos.x = f32::clamp(layout_r.min.x + (layout_r.get_width() - text_size.x) * style.window_title_align.x + text_size.x, layout_r.min.x, layout_r.max.x);
        marker_pos.y = (layout_r.min.y + layout_r.max.y) * 0.5;
        if marker_pos.x > layout_r.min.x
        {
            render_bullet(window.draw_list, marker_pos, get_color_u32_no_alpha(StyleColor::Text));
    ...
    render_text_clipped(layout_r.min, layout_r.max, name, &text_size, style.window_title_align, &clip_r);
}
