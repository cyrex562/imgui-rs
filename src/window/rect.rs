...
pub fn window_rect_abs_to_rel(window: &ImGuiWindow, r: &ImRect) -> ImRect {
    let mut off = window.DC.CursorStartPos;
    ImRect::from_floats(
        r.Min.x - off.x,
        r.Min.y - off.y,
        r.Max.x - off.x,
...
pub unsafe fn ClampWindowRect(window: &mut ImGuiWindow, visibility_rect: &ImRect)
{
    ...
    if g.IO.ConfigWindowsMoveFromTitleBarOnly && (flag_clear(window.Flags, ImGuiWindowFlags_NoTitleBar) || window.DockNodeAsHost.is_some() ){
