...
pub unsafe fn SetKeyboardFocusHere(offset: c_int) {
    ...
    let scroll_flags = if window.Appearing {
    ...
    NavMoveRequestSubmit(
        ...
        scroll_flags,
    ); // FIXME-NAV: Once we refactor tabbing, add LegacyApi flag to not activate non-inputable.
    if offset == -1 {
