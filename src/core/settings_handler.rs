...
pub struct SettingsHandler {
    ...
    pub ReadOpenFn: fn(g: &mut AppContext, handler: *mut Self, name: *const c_char),
    ...
    pub ReadLineFn:
        fn(g: &mut AppContext, handler: *mut Self, entry: *mut c_void, line: *const c_char),

    // c_void        (*ApplyAllFn)(g: &mut ImguiContext, *mut handler: ImGuiSettingsHandler);                                // Read: Called after reading (in registration order)
    pub ApplyAllFn: fn(g: &mut AppContext, handler: *mut Self),
    ...
    pub WriteAllFn: fn(g: &mut AppContext, handler: *mut Self, out_buf: *mut ImGuiTextBuffer),
}
