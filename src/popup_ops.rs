...
use crate::nav_ops::{NavCalcPreferredRefPos, NavMoveRequestTryWrapping, NavRestoreLastChildNavWindow};
use crate::next_window_data_flags::ImGuiNextWindowDataFlags_HasPos;
use crate::popup_data::ImGuiPopupData;
use crate::popup_flags::{ImGuiPopupFlags, ImGuiPopupFlags_AnyPopupId, ImGuiPopupFlags_AnyPopupLevel, ImGuiPopupFlags_MouseButtonMask_, ImGuiPopupFlags_None, ImGuiPopupFlags_NoOpenOverExistingPopup, ImGuiPopupFlags_NoOpenOverItems};
use crate::popup_position_policy::{ImGuiPopupPositionPolicy, ImGuiPopupPositionPolicy_ComboBox, ImGuiPopupPositionPolicy_Default, ImGuiPopupPositionPolicy_Tooltip};
...
use crate::window::window_flags::{ImGuiWindowFlags, ImGuiWindowFlags_AlwaysAutoResize, ImGuiWindowFlags_ChildMenu, ImGuiWindowFlags_ChildWindow, ImGuiWindowFlags_MenuBar, ImGuiWindowFlags_Modal, ImGuiWindowFlags_NoCollapse, ImGuiWindowFlags_NoDocking, ImGuiWindowFlags_NoSavedSettings, ImGuiWindowFlags_NoTitleBar, ImGuiWindowFlags_Popup, ImGuiWindowFlags_Tooltip};
...
pub unsafe fn IsPopupOpen(id: ImGuiID, popup_flags: ImGuiPopupFlags) -> bool
{
    ...
    else
    {
        ...
        else
        {
            ...
            return g.OpenPopupStack.len() > g.BeginPopupStack.len() && g.OpenPopupStack[g.BeginPopupStack.len()].PopupId == id;
        }
    }
}
...
pub unsafe fn OpenPopupEx(id: ImGuiID, popup_flags: ImGuiPopupFlags)
{
    ...
    popup_ref.OpenMousePos = if IsMousePosValid(&g.IO.MousePos) { g.IO.MousePos } else { popup_ref.OpenPopupPos };
...
pub unsafe fn ClosePopupsOverWindow(ref_window: *mut ImGuiWindow, restore_focus_to_window_under_popup: bool)
{
    let g = GImGui; // ImGuiContext& g = *GImGui;
    ...
    if popup_count_to_keep < g.OpenPopupStack.len() // This test is not required but it allows to set a convenient breakpoint on the statement below
    {
...
pub unsafe fn ClosePopupToLevel(remaining: usize, restore_focus_to_window_under_popup: bool)
{
    ...
    g.OpenPopupStack.resize_with(remaining, ImGuiPopupData::default());

    if restore_focus_to_window_under_popup
    {
        let mut focus_window: *mut ImGuiWindow = if is_not_null(popup_window) && flag_set(popup_window.Flags, ImGuiWindowFlags_ChildMenu) { popup_window.ParentWindow } else { popup_backup_nav_window };
...
pub unsafe fn CloseCurrentPopup() {
    ...
    if popup_idx < 0 || popup_idx >= g.OpenPopupStack.len() || g.BeginPopupStack[popup_idx].PopupId != g.OpenPopupStack[popup_idx].PopupId {
        return;
    }
    ...
    while (popup_idx > 0) {
        ...
        if (is_not_null(popup_window) && flag_set(popup_window.Flags, ImGuiWindowFlags_ChildMenu)) {
            if (is_not_null(parent_popup_window) && flag_clear(parent_popup_window.Flags, ImGuiWindowFlags_MenuBar)) {
                close_parent = true;
            }
        }
        if !close_parent {
...
pub unsafe fn BeginPopupModal(name: *const c_char, p_open: *mut bool, mut flags: ImGuiWindowFlags) -> bool
{
    let g = GImGui; // ImGuiContext& g = *GImGui;
    ...
    if !is_open || (is_not_null(p_open) && !*p_open) // NB: is_open can be 'false' when the popup is completely clipped (e.g. zero size display)
    {
        EndPopup();
        if is_open {
...
pub unsafe fn BeginPopupContextWindow(mut str_id: *const c_char, popup_flags: ImGuiPopupFlags) -> bool
{
    let g = GImGui; // ImGuiContext& g = *GImGui;
    let mut window = g.CurrentWindow;
    if !str_id {
...
pub unsafe fn BeginPopupContextVoid(mut str_id: *const c_char, popup_flags: ImGuiPopupFlags) -> bool
{
    let g = GImGui; // ImGuiContext& g = *GImGui;
    let mut window = g.CurrentWindow;
    if (!str_id) {
...
pub unsafe fn FindBestWindowPosForPopupEx(ref_pos: &ImVec2, size: &ImVec2, last_dir: *mut ImGuiDir, r_outer: &mut ImRect, r_avoid: &ImRect, policy: ImGuiPopupPositionPolicy) -> ImVec2
{
    ...
    {
        let dir_prefered_order: [ImGuiDir;4] = [ ImGuiDir_Down, ImGuiDir_Right, ImGuiDir_Left, ImGuiDir_Up ];
        // for (let n: c_int = (*last_dir != ImGuiDir_None) ? -1 : 0; n < ImGuiDir_COUNT; n++)
        for n in if *last_idr != ImGuiDir_None { -1 } else {0} .. ImGuiDir_COUNT
        {
            const dir: ImGuiDir = if (n == -1) { *last_dir } else { dir_prefered_order[n] };
    ...
    {
        let dir_prefered_order: [ImGuiDir;4] = [ ImGuiDir_Right, ImGuiDir_Down, ImGuiDir_Up, ImGuiDir_Left ];
        // for (let n: c_int = (*last_dir != ImGuiDir_None) ? -1 : 0; n < ImGuiDir_COUNT; n++)
         for n in if *last_idr != ImGuiDir_None { -1 } else {0} .. ImGuiDir_COUNT
        {
            let dir: ImGuiDir = if n == -1 { *last_dir } else { dir_prefered_order[n] };
            if n != -1 && dir == *last_dir { // Already tried this direction?
                continue;
            }

            let avail_w: c_float =  (if dir == ImGuiDir_Left { r_avoid.Min.x } else { r_outer.Max.x }) - (if dir == ImGuiDir_Right { r_avoid.Max.x } else { r_outer.Min.x });
            let avail_h: c_float =  (if dir == ImGuiDir_Up { r_avoid.Min.y } else { r_outer.Max.y }) - (if dir == ImGuiDir_Down { r_avoid.Max.y } else { r_outer.Min.y });
            ...
            pos.x = if dir == ImGuiDir_Left { r_avoid.Min.x - size.x } else {
                if dir == ImGuiDir_Right {
                    r_avoid.Max.x
                } else { base_pos_clamped.x }
            };
            pos.y = if dir == ImGuiDir_Up { r_avoid.Min.y - size.y } else {
...
pub unsafe fn FindBestWindowPosForPopup(window: *mut ImGuiWindow) -> ImVec2
{
    ...
    if window.Flags & ImGuiWindowFlags_ChildMenu
    {
        ...
        if parent_window.DC.MenuBarAppending {
            r_avoid = ImRect(-f32::MAX, parent_window.ClipRect.Min.y, f32::MAX, parent_window.ClipRect.Max.y);
        ...
        return FindBestWindowPosForPopupEx(&window.Pos, &window.Size, &mut window.AutoPosLastDirection, &mut r_outer, &r_avoid, ImGuiPopupPositionPolicy_Default);
    }
    if flag_set(window.Flags, ImGuiWindowFlags_Popup)
    {
        return FindBestWindowPosForPopupEx(&window.Pos, &window.Size, &mut window.AutoPosLastDirection, &mut r_outer, &ImRect::from_vec2(&window.Pos, &window.Pos), ImGuiPopupPositionPolicy_Default); // Ideally we'd disable r_avoid here
    }
    if flag_set(window.Flags, ImGuiWindowFlags_Tooltip)
    {
        ...
        let mut r_avoid: ImRect = ImRect::default();
        if !g.NavDisableHighlight && g.NavDisableMouseHover && flag_clear(g.IO.ConfigFlags, ImGuiConfigFlags_NavEnableSetMousePos) {
        ...
        return FindBestWindowPosForPopupEx(&ref_pos, &window.Size, &mut window.AutoPosLastDirection, &mut r_outer, &r_avoid, ImGuiPopupPositionPolicy_Tooltip);
    }
    // IM_ASSERT(0);
    return window.Pos;
}
