...
use crate::type_defs::{ImPoolIdx, ImguiHandle};
use libc::c_int;
...
impl ImPool<T> {
    ...
    // *mut T          GetByKey(ImguiHandle key)               { idx: c_int = Map.GetInt(key, -1); return (idx != -1) ? &Buf[idx] : NULL; }
    pub fn GetByKey(&mut self, key: ImguiHandle) -> *mut T {
        let idx = self.Map.GetInt(key, -1);
        return if idx != -1 { &mut self.Buf[idx] } else { None };
    }
    ...
    // *mut T          GetOrAddByKey(ImguiHandle key)          { *mut p_idx: c_int = Map.GetIntRef(key, -1); if (*p_idx != -1) return &Buf[*p_idx]; *p_idx = FreeIdx; return Add(); }
    pub unsafe fn GetOrAddByKey(&mut self, key: ImguiHandle) -> *mut T {
    ...
    // void        Remove(ImguiHandle key, *const T p)     { Remove(key, GetIndex(p)); }
    pub fn Remove(&mut self, key: ImguiHandle, p: *const T) {
        self.Remove2(key, self.GetIndex(p));
    }

    // void        Remove(ImguiHandle key, ImPoolIdx idx)  { Buf[idx].~T(); *(*mut c_int)&Buf[idx] = FreeIdx; FreeIdx = idx; Map.SetInt(key, -1); AliveCount-= 1; }
    pub fn Remove2(&mut self, key: ImguiHandle, idx: ImPoolIdx) {
        todo!()
    }
