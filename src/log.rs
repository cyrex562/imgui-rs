...
pub const ImGuiDebugLogFlags_EventMask: u32 = ImGuiDebugLogFlags::EventActiveId
...
pub unsafe fn LogRenderedText(ref_pos: &Vector2D, text: &str, mut text_end: &str) {
    ...
    let log_new_line = ref_pos != Vector2D::new(0.0, 0.0)
        && (ref_pos.y > g.log_line_pos_y + g.style.frame_padding.y + 1);
    if (ref_pos) {
    ...
    loop {
        ...
        if line_start != line_end || !is_last_line {
            ...
            let indentation = if (g.LogLineFirstItem) {
                tree_depth * 4
            } else {
                1
            };
