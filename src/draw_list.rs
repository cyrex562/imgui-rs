...
use crate::constants::{IM_DRAWLIST_ARCFAST_SAMPLE_MAX, IM_DRAWLIST_ARCFAST_TABLE_SIZE, IM_DRAWLIST_CIRCLE_AUTO_SEGMENT_MAX};
use crate::draw::ImDrawCallback;
use crate::draw_cmd::ImDrawCmd;
use crate::draw_cmd_header::ImDrawCmdHeader;
use crate::draw_flags::{FixRectCornerFlags, ImDrawFlags, ImDrawFlags_Closed, ImDrawFlags_RoundCornersBottom, ImDrawFlags_RoundCornersBottomLeft, ImDrawFlags_RoundCornersBottomRight, ImDrawFlags_RoundCornersLeft, ImDrawFlags_RoundCornersMask_, ImDrawFlags_RoundCornersNone, ImDrawFlags_RoundCornersRight, ImDrawFlags_RoundCornersTop, ImDrawFlags_RoundCornersTopLeft, ImDrawFlags_RoundCornersTopRight};
use crate::draw_list_flags::{ImDrawListFlags, ImDrawListFlags_AllowVtxOffset, ImDrawListFlags_AntiAliasedFill, ImDrawListFlags_AntiAliasedLines, ImDrawListFlags_AntiAliasedLinesUseTex, ImDrawListFlags_None};
...
use crate::math_ops::{ImBezierCubicCalc, ImBezierQuadraticCalc, ImCeil, ImClamp, ImCos, ImFabs, ImMax, ImMin, ImSin, PathBezierCubicCurveToCasteljau, PathBezierQuadraticCurveToCasteljau};
...
impl ImDrawList {
    ...
    pub unsafe fn PopClipRect(&mut self) {
        ...
        self._CmdHeader.ClipRect = if self._ClipRectStack.len() == 0 { self._Data.ClipRectFullscreen } else { self._ClipRectStack[self._ClipRectStack.len() - 1] };
        self._OnChangedClipRect();
    }
...
// void  AddLine(const ImVec2& p1, const ImVec2& p2, col: u32, c_float thickness = 1f32);
    ...
    pub unsafe fn AddRect(&mut self, p_min: &ImVec2, p_max: &ImVec2, col: u32, rounding: c_float, flags: ImDrawFlags, thickness: c_float) {
        ...
        if flag_set(Flags, ImDrawListFlags_AntiAliasedLines) {
            self.PathRect(p_min + ImVec2::new(0.50, 0.500), p_max - ImVec2::new(0.50, 0.500), rounding, flags);
        }
        else {
            self.PathRect(p_min + ImVec2::new(0.50, 0.500), p_max - ImVec2::new(0.49, 0.490), rounding, flags); // Better looking lower-right corner and rounded non-AA shapes.
    ...
    // void  AddRectFilledMultiColor(const ImVec2& p_min, const ImVec2& p_max, col_upr_left: u32, col_upr_right: u32, col_bot_right: u32, col_bot_left: u32);
    ...
    // void  AddQuadFilled(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, col: u32);
    pub unsafe fn AddQuadFilled(&mut self, p1: &ImVec2, p2: &ImVec2, p3: &ImVec2, p4: &ImVec2, col: u32) {
        if (col & IM_COL32_A_MASK) == 0 {
            return;
        }
    ...
    // void  AddTriangleFilled(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, col: u32);
    ...
    // void  AddCircleFilled(const ImVec2& center, c_float radius, col: u32, num_segments: c_int = 0);
    pub unsafe fn AddCircleFilled(&mut self, center: &ImVec2, radius: c_float, col: u32, mut num_segments: size_t) {
        if (col & IM_COL32_A_MASK) == 0 || radius < 0.5 {
            return;
        }
    ...
    // void  AddNgonFilled(const ImVec2& center, c_float radius, col: u32, num_segments: c_int);
    pub unsafe fn AddNgonFilled(&mut self, center: &ImVec2, radius: c_float, col: u32, num_segments: size_t) {
        if (col & IM_COL32_A_MASK) == 0 || num_segments <= 2 {
            return;
        }
    ...
    // void  AddText(const ImVec2& pos, col: u32, const char* text_begin, const char* text_end = NULL);
...
// text_end = NULL, c_float wrap_width = 0f32, const ImVec4* cpu_fine_clip_rect = NULL);
    ...
    pub unsafe fn AddPolyline(&mut self, points: *const ImVec2, points_count: size_t, col: u32, flags: ImDrawFlags, thickness: c_float) {
        ...
        let count: size_t = if closed { points_count }else { points_count - 1 }; // The number of line segments we need to draw
        ...
        if flag_set(self.lags, ImDrawListFlags_AntiAliasedLines)
        {
            ...
            let use_texture: bool = flag_set(self.Flags, ImDrawListFlags_AntiAliasedLinesUseTex) && (integer_thickness < IM_DRAWLIST_TEX_LINES_WIDTH_MAX) && (fractional_thickness <= 0.000010) && (AA_SIZE == 1.0);

            // We should never hit this, because NewFrame() doesn't set ImDrawListFlags_AntiAliasedLinesUseTex unless ImFontAtlasFlags_NoBakedLines is off
            // IM_ASSERT_PARANOID(!use_texture || !(_Data.Font->ContainerAtlas->Flags & ImFontAtlasFlags_NoBakedLines));

            let idx_count: size_t = if use_texture { count * 6 } else if  thick_line { count * 18 } else { count * 12 };
            let vtx_count: size_t = if use_texture { points_count * 2 } else if thick_line { points_count * 4 } else { points_count * 3 };
            self.PrimReserve(idx_count, vtx_count);

            // Temporary buffer
            // The first <points_count> items are normals at each line point, then after that there are either 2 or 4 temp points for each line point
            let count = if use_texture == true || thick_line == false { 3 } else { 5 };
            let mut temp_normals: *mut ImVec2 = libc::malloc(points_count * count * mem::size_of::<ImVec2>()); //-V630
            ...
            if use_texture || !thick_line
            {
                ...
                let half_draw_size: c_float =  if use_texture { ((thickness * 0.5) + 1) } else { AA_SIZE };
                ...
                for i1 in 0 .. count
                {
                    ...
                    let mut idx2: size_t = if (i1 + 1) == points_count { self._VtxCurrentIdx } else {
                        idx1 + (if use_texture == true { 2 } else { 3 }) }; // Vertex index for end of segment
                    ...
                    if use_texture
                    {
                        ...
                        self._IdxWritePtr[0] = (idx2 + 0); self._IdxWritePtr[1] = (idx1 + 0); self._IdxWritePtr[2] = (idx1 + 1); // Right tri
                        self._IdxWritePtr[3] = (idx2 + 1); self._IdxWritePtr[4] = (idx1 + 1); self._IdxWritePtr[5] = (idx2 + 0); // Left tri
                        self._IdxWritePtr += 6;
                    }
                    else
                    {
                        // Add indexes for four triangles
                        self._IdxWritePtr[0] = (idx2 + 0); self._IdxWritePtr[1] = (idx1 + 0); self._IdxWritePtr[2] = (idx1 + 2); // Right tri 1
                        self._IdxWritePtr[3] = (idx1 + 2); self._IdxWritePtr[4] = (idx2 + 2); self._IdxWritePtr[5] = (idx2 + 0); // Right tri 2
                        self._IdxWritePtr[6] = (idx2 + 1); self._IdxWritePtr[7] = (idx1 + 1); self._IdxWritePtr[8] = (idx1 + 0); // Left tri 1
                        self._IdxWritePtr[9] = (idx1 + 0); self._IdxWritePtr[10] = (idx2 + 0); self._IdxWritePtr[11] = (idx2 + 1); // Left tri 2
                ...
                if use_texture
                {
                    ...
                    {
                        self._VtxWritePtr[0].pos = temp_points[i * 2 + 0]; self._VtxWritePtr[0].uv = tex_uv0; self._VtxWritePtr[0].col = col; // Left-side outer edge
                        self._VtxWritePtr[1].pos = temp_points[i * 2 + 1]; self._VtxWritePtr[1].uv = tex_uv1; self._VtxWritePtr[1].col = col; // Right-side outer edge
                        self._VtxWritePtr += 2;
                    }
                }
                else
                {
                    ...
                    for i in 0.. points_count
                    {
                        self._VtxWritePtr[0].pos = points[i];              self._VtxWritePtr[0].uv = opaque_uv; self._VtxWritePtr[0].col = col;       // Center of line
                        self._VtxWritePtr[1].pos = temp_points[i * 2 + 0]; self._VtxWritePtr[1].uv = opaque_uv; self._VtxWritePtr[1].col = col_trans; // Left-side outer edge
                        self._VtxWritePtr[2].pos = temp_points[i * 2 + 1]; self._VtxWritePtr[2].uv = opaque_uv; self._VtxWritePtr[2].col = col_trans; // Right-side outer edge
                        self._VtxWritePtr += 3;
                    }
                }
            }
            else
            {
                ...
                {
                    let i2: c_int = if (i1 + 1) == points_count {0} else { i1 + 1 }; // i2 is the second point of the line segment
                     let mut idx2: size_t =  if (i1 + 1) == points_count { self._VtxCurrentIdx } else{ idx1 + 4 }; // Vertex index for end of segment
                    ...
                    // Add indexes
                    self._IdxWritePtr[0]  = (idx2 + 1); self._IdxWritePtr[1]  = (idx1 + 1); self._IdxWritePtr[2]  = (idx1 + 2);
                    self._IdxWritePtr[3]  = (idx1 + 2); self._IdxWritePtr[4]  = (idx2 + 2); self._IdxWritePtr[5]  = (idx2 + 1);
                    self._IdxWritePtr[6]  = (idx2 + 1); self._IdxWritePtr[7]  = (idx1 + 1); self._IdxWritePtr[8]  = (idx1 + 0);
                    self._IdxWritePtr[9]  = (idx1 + 0); self._IdxWritePtr[10] = (idx2 + 0); self._IdxWritePtr[11] = (idx2 + 1);
                    self._IdxWritePtr[12] = (idx2 + 2); self._IdxWritePtr[13] = (idx1 + 2); self._IdxWritePtr[14] = (idx1 + 3);
                    self._IdxWritePtr[15] = (idx1 + 3); self._IdxWritePtr[16] = (idx2 + 3); self._IdxWritePtr[17] = (idx2 + 2);
                ...
                for i in 0 .. points_count
                {
                    self._VtxWritePtr[0].pos = temp_points[i * 4 + 0]; self._VtxWritePtr[0].uv = opaque_uv; self._VtxWritePtr[0].col = col_trans;
                    self._VtxWritePtr[1].pos = temp_points[i * 4 + 1]; self._VtxWritePtr[1].uv = opaque_uv; self._VtxWritePtr[1].col = col;
                    self._VtxWritePtr[2].pos = temp_points[i * 4 + 2]; self._VtxWritePtr[2].uv = opaque_uv; self._VtxWritePtr[2].col = col;
                    self._VtxWritePtr[3].pos = temp_points[i * 4 + 3]; self._VtxWritePtr[3].uv = opaque_uv; self._VtxWritePtr[3].col = col_trans;
                    self._VtxWritePtr += 4;
                }
            }
        ...
        else
        {
            ...
            for i1 in 0 .. count
            {
                ...
                self._VtxWritePtr[0].pos.x = p1.x + dy; self._VtxWritePtr[0].pos.y = p1.y - dx; self._VtxWritePtr[0].uv = opaque_uv; self._VtxWritePtr[0].col = col;
                self._VtxWritePtr[1].pos.x = p2.x + dy; self._VtxWritePtr[1].pos.y = p2.y - dx; self._VtxWritePtr[1].uv = opaque_uv; self._VtxWritePtr[1].col = col;
                self._VtxWritePtr[2].pos.x = p2.x - dy; self._VtxWritePtr[2].pos.y = p2.y + dx; self._VtxWritePtr[2].uv = opaque_uv; self._VtxWritePtr[2].col = col;
                self._VtxWritePtr[3].pos.x = p1.x - dy; self._VtxWritePtr[3].pos.y = p1.y + dx; self._VtxWritePtr[3].uv = opaque_uv; self._VtxWritePtr[3].col = col;
                self._VtxWritePtr += 4;

                self._IdxWritePtr[0] = (self._VtxCurrentIdx); self._IdxWritePtr[1] = (self._VtxCurrentIdx + 1); self._IdxWritePtr[2] = (self._VtxCurrentIdx + 2);
                self._IdxWritePtr[3] = (self._VtxCurrentIdx); self._IdxWritePtr[4] = (self._VtxCurrentIdx + 2); self._IdxWritePtr[5] = (self._VtxCurrentIdx + 3);
                self._IdxWritePtr += 6;
    ...
    pub unsafe fn AddConvexPolyFilled(&mut self, points: *const ImVec2, points_count: size_t, col: u32) {
        if points_count < 3 {
            return;
        }
        ...
        if flag_set(self.Flags, ImDrawListFlags_AntiAliasedFill)
        {
            ...
            // Compute normals
            let mut temp_normals: *mut ImVec2 =libc::malloc(points_count * mem::size_of::<ImVec2>()); //-V630
            ...
            while i1 < points_count
            {
                ...
                // Add vertices
                self._VtxWritePtr[0].pos.x = (points[i1].x - dm_x); self._VtxWritePtr[0].pos.y = (points[i1].y - dm_y); self._VtxWritePtr[0].uv = uv; self._VtxWritePtr[0].col = col;        // Inner
                self._VtxWritePtr[1].pos.x = (points[i1].x + dm_x); self._VtxWritePtr[1].pos.y = (points[i1].y + dm_y); self._VtxWritePtr[1].uv = uv; self._VtxWritePtr[1].col = col_trans;  // Outer
                self._VtxWritePtr += 2;
        ...
        else
        {
            ...
            {
                self._VtxWritePtr[0].pos = points[i]; self._VtxWritePtr[0].uv = uv; self._VtxWritePtr[0].col = col;
                self._VtxWritePtr+= 1;
            }
            // for (let i: c_int = 2; i < points_count; i++)
            for i in 2 .. points_count
    ...
    // void  AddBezierCubic(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, col: u32, c_float thickness, num_segments: c_int = 0); // Cubic Bezier (4 control points)
    pub unsafe fn AddBezierCubic(&mut self, p1: &ImVec2, p2: &ImVec2, p3: &ImVec2, p4: &ImVec2, col: u32, thickness: c_float, num_segments: c_int) {
        if (col & IM_COL32_A_MASK) == 0 {
            return;
        }
    ...
    // void  AddBezierQuadratic(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, col: u32, c_float thickness, num_segments: c_int = 0);               // Quadratic Bezier (3 control points)
    pub unsafe fn AddBezierQuadratic(&mut self, p1: &ImVec2, p2: &ImVec2, p3: &ImVec2, col: u32, thickness: c_float, num_segments: c_int) {
        if (col & IM_COL32_A_MASK) == 0 {
            return;
        }
...
// void  AddImage(ImTextureID user_texture_id, const ImVec2& p_min, const ImVec2& p_max, const ImVec2& uv_min = ImVec2::new(0, 0), const ImVec2& uv_max = ImVec2::new(1, 1), col: u32 = IM_COL32_WHITE);
    ...
    // void  AddImageQuad(ImTextureID user_texture_id, const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, const ImVec2& uv1 = ImVec2::new(0, 0), const ImVec2& uv2 = ImVec2::new(1, 0), const ImVec2& uv3 = ImVec2::new(1, 1), const ImVec2& uv4 = ImVec2::new(0, 1), col: u32 = IM_COL32_WHITE);
...
// inline    void  PathClear()                                                 { _Path.Size = 0; }
    ...
    // void  PathArcTo(const ImVec2& center, c_float radius, c_float a_min, c_float a_max, num_segments: c_int = 0);
    pub fn PathArcTo(&mut self, center: &ImVec2, radius: c_float, a_min: c_float, a_max: c_float, num_segments: size_t) {
        if radius < 0.5
        {
        ...
        if radius <= self._Data.ArcFastRadiusCutof0
        {
            ...
            let a_min_sample: c_float = if a_is_reverse { ImFloor(a_min_sample_0) } else { ImCeil(a_min_sample_0) };
            let a_max_sample: c_float = if a_is_reverse { ImCeil(a_max_sample_0) } else { ImFloor(a_max_sample_0) };
            let a_mid_samples: c_float = if a_is_reverse { ImMax(a_min_sample - a_max_sample, 0.0) } else { ImMax(a_max_sample - a_min_sample, 0.0) };

            let a_min_segment_angle: c_float =  a_min_sample * IM_PI * 2.0 / IM_DRAWLIST_ARCFAST_SAMPLE_MAX;
            let a_max_segment_angle: c_float =  a_max_sample * IM_PI * 2.0 / IM_DRAWLIST_ARCFAST_SAMPLE_MAX;
        ...
        else
        {
            ...
            let arc_segment_count: c_float = ImMax(ImCeil(circle_segment_count * arc_length / (IM_PI * 2.00)), (2.0 * IM_PI / arc_length));
            self._PathArcToN(center, radius, a_min, a_max, arc_segment_count as size_t);
        }
    }


    // void  PathArcToFast(const ImVec2& center, c_float radius, a_min_of_12: c_int, a_max_of_12: c_int);                // Use precomputed angles for a 12 steps circle
    ...
    // void  PathBezierCubicCurveTo(const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, num_segments: c_int = 0); // Cubic Bezier (4 control points)
    pub fn PathBezierCubicCurveTo(&mut self, p2: &ImVec2, p3: &ImVec2, p4: &ImVec2, num_segments: c_int) {
        ...
        if num_segments == 0
        {
            PathBezierCubicCurveToCasteljau(&mut self._Path, p1.x, p1.y, p2.x, p2.y, p3.x, p3.y, p4.x, p4.y, self._Data.CurveTessellationTol, 0); // Auto-tessellated
        }
        else
        {
    ...
    // void  PathBezierQuadraticCurveTo(const ImVec2& p2, const ImVec2& p3, num_segments: c_int = 0);               // Quadratic Bezier (3 control points)
    pub fn PathBezierQuadraticCurveTo(&mut self, p2: &ImVec2, p3: &ImVec2, num_segments: c_int)
    {
        ...
        if num_segments == 0
        {
            PathBezierQuadraticCurveToCasteljau(&mut self._Path, p1.x, p1.y, p2.x, p2.y, p3.x, p3.y, self._Data.CurveTessellationTol, 0);// Auto-tessellated
        }
        else
        {
            let t_step: c_float =  1.0 / num_segments;
    ...
    pub fn PathRect(&mut self, rect_min: &ImVec2, rect_max: &ImVec2, mut rounding: c_float, mut flags: ImDrawFlags) {
        ...
        {
            let rounding_tl: c_float =  if flag_set(flags, ImDrawFlags_RoundCornersTopLeft) { rounding } else { 0 };
...
// void  PrimReserve(idx_count: c_int, vtx_count: c_int);
    pub fn PrimReserve(&mut self, idx_count: size_t, vtx_count: size_t) {
        ...
        if mem::size_of::<ImDrawIdx>() == 2 && (self._VtxCurrentIdx + vtx_count >= (1 << 16)) && flag_set(self.Flags, ImDrawListFlags_AllowVtxOffset) {
        ...
        self.VtxBuffer.resize_with(vtx_buffer_old_size + vtx_count, ImDrawVert::default());
        self._VtxWritePtr = self.VtxBuffer.Data + vtx_buffer_old_size;

        let idx_buffer_old_size: size_t = self.IdxBuffer.len();
        self.IdxBuffer.resize_with(idx_buffer_old_size + idx_count, 0);
    ...
    // void  PrimRect(const ImVec2& a, const ImVec2& b, col: u32);      // Axis aligned rectangle (composed of two triangles)
    ...
    // void  PrimRectUV(const ImVec2& a, const ImVec2& b, const ImVec2& uv_a, const ImVec2& uv_b, col: u32);
    ...
    // void  PrimQuadUV(const ImVec2& a, const ImVec2& b, const ImVec2& c, const ImVec2& d, const ImVec2& uv_a, const ImVec2& uv_b, const ImVec2& uv_c, const ImVec2& uv_d, col: u32);
    ...
    // inline    void  PrimWriteVtx(const ImVec2& pos, const ImVec2& uv, col: u32)    { _VtxWritePtr->pos = pos; _VtxWritePtr->uv = uv; _VtxWritePtr->col = col; _VtxWritePtr+= 1; _VtxCurrentIdx+= 1; }
    ...
    // inline    void  PrimVtx(const ImVec2& pos, const ImVec2& uv, col: u32)         { PrimWriteIdx((ImDrawIdx)_VtxCurrentIdx); PrimWriteVtx(pos, uv, col); } // Write vertex with unique index
...
// inline    void  AddBezierCurve(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, col: u32, c_float thickness, num_segments: c_int = 0) { AddBezierCubic(p1, p2, p3, p4, col, thickness, num_segments); } // OBSOLETED in 1.80 (Jan 2021)
...
// inline    void  PathBezierCurveTo(const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, num_segments: c_int = 0) { PathBezierCubicCurveTo(p2, p3, p4, num_segments); } // OBSOLETED in 1.80 (Jan 2021)
...
// Note that this leaves the ImDrawList in a state unfit for further commands, as most code assume that CmdBuffer.Size > 0 && CmdBuffer.back().UserCallback == NULL
    ...
    // void  _PathArcToFastEx(const ImVec2& center, c_float radius, a_min_sample: c_int, a_max_sample: c_int, a_step: c_int);
    pub fn _PathArcToFastEx(&mut self, center: &ImVec2, radius: c_float, a_min_sample: size_t, a_max_sample: size_t, mut a_step: size_t) {
        if radius < 0.5
        {
            self._Path.push(center.clone());
            return;
        ...
        self._Path.resize_with(self._Path.Size + samples, ImVec2::default());
    ...
    // void  _PathArcToN(const ImVec2& center, c_float radius, c_float a_min, c_float a_max, num_segments: c_int);
    pub fn _PathArcToN(&mut self, center: &ImVec2, radius: c_float, a_min: c_float, a_max: c_float, num_segments: size_t) {
        if radius < 0.5
        {
            self._Path.push(center.clone());
            return;
