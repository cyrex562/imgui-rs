...
// void  PrimReserve(idx_count: c_int, vtx_count: c_int);
    ...
    // inline    void  PrimWriteVtx(const pos: &ImVec2, const uv: &ImVec2, u32 col)    { _VtxWritePtr.pos = pos; _VtxWritePtr.uv = uv; _VtxWritePtr.col = col; _VtxWritePtr+= 1; _VtxCurrentIdx+= 1; }
    ...
    // inline    void  PrimVtx(const pos: &ImVec2, const uv: &ImVec2, u32 col)         { PrimWriteIdx((ImDrawIdx)_VtxCurrentIdx); PrimWriteVtx(pos, uv, col); } // Write vertex with unique index
