...
impl DrawListSharedData {
    pub fn new() -> Self {
        ...
        out.arc_fast_radius_cutoff = drawlist_circle_auto_segment_calc_r(
            DRAW_LIST_ARCFAST_SAMPLE_MAX as f32,
            out.circle_segment_max_error,
        );
    ...
    pub fn set_circle_tesselation_max_error(&mut self, max_error: f32) {
        ...
        for i in 0..self.circle_segment_counts.len() {
            ...
            self.circle_segment_counts[i] = if i > 0 {
                u8::from(drawlist_circle_auto_segment_calc(
                    radius,
                    self.circle_segment_max_error,
                ))
            } else {
