...
pub fn dock_settings_find_node_settings(g: &mut Context, id: Id32) -> Option<&mut DockNodeSettings>
{
    ...
    for n in 0..dc.nodes_settings.len()
    {
        if dc.nodes_settings[n].id == id {
            return Some(&mut dc.nodes_settings[n]);
        }
    }
    return None;
}
...
pub fn dock_settings_handler_apply_all(g: &mut Context, handler: &mut SettingsHandler)
{
    ...
    ImGuiDockContext* dc  = &g.dock_context;
    if g.windows.len() == 0 {
        dock_context_prune_unused_settings_nodes(g);
    }
    dock_context_build_nodes_from_settings(g, dc.nodes_settings.data, dc.nodes_settings.size);
    dock_context_build_add_windows_to_nodes(g, 0);
}
...
pub fn dock_settings_handler_read_line(g: &mut Context, handler: &mut SettingsHandler, data: &Vec<u8>, line: &mut String)
{
    // // char c = 0;
    // let mut c: u8 = 0u8;
    // // int x = 0, y = 0;
    ...
    // let mut node: DockNodeSettings = Default::default();
    // *line = string_skip_blank(line);
    // if strncmp(line, "dock_node", 8) == 0 { line = string_skip_blank(line + strlen("dock_node")); }
    // else if strncmp(line, "DockSpace", 9) == 0 { line = string_skip_blank(line + strlen("DockSpace")); node.flags |= DockNodeFlags::DockSpace; }
    // else { return; };
    ...
    // g.dock_context.nodes_settings.push_back(node);
    todo!()
}
...
pub fn dock_settings_handler_dock_node_to_settings(g: &mut Context, dc: &mut DockContext, node: &mut DockNode, depth: i32)
{
    ...
    // dc.nodes_settings.push_back(node_settings);
...
//     dc.nodes_settings.resize(0);
//     dc.nodes_settings.reserve(dc.Nodes.data.size);
...
//
//     int max_depth = 0;
//     for (int node_n = 0; node_n < dc.nodes_settings.size; node_n += 1)
//         max_depth = ImMax(dc.nodes_settings[node_n].Depth, max_depth);
//
