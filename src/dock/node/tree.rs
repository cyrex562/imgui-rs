...
pub fn dock_node_tree_merge(g: &mut Context, parent_node: &mut DockNode, merge_lead_child: Option<&mut DockNode>)
{
    ...
    if child_0.is_some() {
        parent_node.local_flags_in_windows = parent_node.local_flags_in_windows.bitor(&child_0.unwrap().local_flags_in_windows);
    } else {
        parent_node.local_flags_in_windows.clear();
    }

    if child_1.is_some() {
        parent_node.local_flags_in_windows = parent_node.local_flags_in_windows.bitor(&child_1.unwrap().local_flags_in_windows);
    } else {
        parent_node.local_flags_in_windows.clear();
    }
