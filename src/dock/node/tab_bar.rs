...
use crate::dock::node;
use crate::dock::node::{DockNode, DockNodeFlags, title_bar, window};
...
use crate::rect::Rect;
use crate::style::{color_convert_u32_to_float4, get_color_u32_no_alpha, pop_style_color, push_style_color, WINDOW_DOCK_STYLE_COLORS};
...
pub fn dock_node_update_tab_bar(g: &mut Context, node: &mut DockNode, host_window: &mut Window)
{
    ...
    // Begin tab bar
    let mut tab_bar_flags: HashSet<TabBarFlags> = HashSet::from([TabBarFlags::Reorderable, TabBarFlags::AutoSelectNewTabs, TabBarFlags::SaveSettings, TabBarFlags::DockNode]); // | ImGuiTabBarFlags_NoTabListScrollingButtons);
    ...
    // Backup style colors
    let mut backup_style_colors: [Vector4D;WindowDockStyleColor.len()] = [Vector4D::default();WindowsDockStyleColor.len()];
    ...
    for win_id in node.windows
    {
        ...
        if (closed_all || closed_one) == (window.tab_id != INVALID_ID && window.has_close_button )&& !(window.flags.contains(&WindowFlags::UnsavedDocument)) {
            continue;
        }
        if window.last_frame_active + 1 >= g.frame_count || !node_was_active
        {
            ...
            tab_item_flags = add_hash_set(&tab_item_flags,&window.window_class.tab_item_flags_override_set);
            if window.flags.contains(&WindowFlags::UnsavedDocument) {
                tab_item_flags.insert(TabItemFlags::UnsavedDocument);
            }
            if tab_bar.flags.contains(&TabBarFlags::NoCloseWithMiddleMouseButton) {
            ...
            for color_n in 0 .. WindowDockStyleColor::LastItem {
                g.style.colors[WINDOW_DOCK_STYLE_COLORS[color_n].clone()] = color_convert_u32_to_float4(window.dock_style.colors[color_n]);
            }

            // Note that TabItemEx() calls TabBarCalcTabID() so our tab item id will ignore the current id stack (rightly so)
            let mut tab_open = true;
            tab_item_ex(tab_bar, &window.name, if window.has_close_button { Some(&tab_open) } else {None}, tab_item_flags, window);
            if !tab_open {
    ...
    for color_n in 0 .. WindowDockStyleColor::LastItem
    {
        g.style.colors[WINDOW_DOCK_STYLE_COLORS[color_n].clone()] = backup_style_colors[color_n].clone();
    }
    ...
    let close_button_is_enabled = node.has_close_button && node.visible_window_id != INVALID_ID && node.visible_window_id.has_close_button;
    ...
    if close_button_is_visible
    {
        if !close_button_is_enabled
        {
            ...
            push_style_color(g, StyleColor::Text, style.colors[StyleColor::Text] * Vector4D(1.0, 1.0, 1.0, 0.4));
        }
    ...
    if g.hovered_id == 0 || g.hovered_id == title_bar_id || g.active_id == title_bar_id
    {
        ...
        if held
        {
            ...
            if tab.is_some() {
                start_mouse_moving_window_or_node(g,
                                                  if tab.unwrap().wwindow_id != INVALID_ID {
                        g.get_window(tab.window_id) } else {
...
pub fn dock_node_calc_tab_bar_layout(g: &mut Context, node: &mut DockNode, mut out_title_rect: Option<&mut Rect>, out_tab_bar_rect: &mut Rect, out_window_menu_button_pos: &mut Vector2D, out_close_button_pos: Option<&mut Vector2D>)
{
    // ImGuiContext& g = *GImGui;
    // ImGuiStyle& style = g.style;
    let style = &mut g.style;

    let mut r= Rect::from((node.pos.x, node.pos.y, node.pos.x + node.size.x, node.pos.y + g.font_size + g.style.frame_padding.y * 2.0));
    if out_title_rect.is_some() { out_title_rect.unwrap().min = r.min.clone(); out_title_rect.unwrap().max = r.max.clone(); }
    ...
    if out_tab_bar_rect { out_tab_bar_rect.unwrap().min = r.min.clone(); out_tab_bar_rect.unwrap().max = r.max.clone() }
