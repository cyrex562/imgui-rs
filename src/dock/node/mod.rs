...
pub fn dock_node_update(g: &mut Context, node: &mut DockNode)
{
    ...
    if node.is_root_node() && host_window.is_some()
    {
        ...
        tree::dock_node_tree_update_pos_size(g, node, &host_window.unwrap().pos, &host_window.unwrap().size, None);
...
pub fn dock_node_tree_update_splitter(g: &mut Context, node: &mut DockNode)
{
    ...
    if child_0.unwrap().is_visible && child_1.unwrap().is_visible
    {
        ...
        let no_resize_axis_flag = if axis == Axis::X { DockNodeFlags::NoResizeX } else { DockNodeFlags::NoResizeY; };
        if (&merged_flags.contains(&DockNodeFlags::NoResize)) || (&merged_flags.contains(&no_resize_axis_flag))
        {
            ...
            window.draw_list.add_rect_filled(bb.min, bb.max, get_color_u32_no_alpha(StyleColor::Separator), g.style.frame_rounding);
        }
        else
        {
            ...
            if (SplitterBehavior(bb, GetID("##splitter"), axis, &cur_size_0, &cur_size_1, min_size_0, min_size_1, WINDOWS_HOVER_PADDING, WINDOWS_RESIZE_FROM_EDGES_FEEDBACK_TIMER, bg_col))
            {
                if (touching_nodes[0].size > 0 && touching_nodes[1].size > 0)
                {
                    ...
                    tree::dock_node_tree_update_pos_size(child_0, child_0.pos, child_0.size);
                    tree::dock_node_tree_update_pos_size(child_1, child_1.pos, child_1.size);
    ...
    if child_0.is_visible {
        dock_node_tree_update_splitter(g, child_0);
    }
    if child_1.is_visible {
        dock_node_tree_update_splitter(g, child_1);
    }
}
