...
pub fn dock_node_calc_split_rects(g: &mut Context, pos_old: &Vector2D, size_old: &Vector2D, size_new: &Vector2D, pos_new: &Vector2D, dir: Direction, size_new_desired: Vector2D)
{
    ...
    let axis = if dir == Direction::Left || dir == Direction::Right {  Axis::X} else { Axis::Y};
...
pub fn dock_node_calc_drop_rects_and_test_mouse_pos(g: &mut Context, parent: &Rect, dir: &Direction, out_r: &mut Rect, outer_docking: bool, test_mouse_pos: Option<&mut Vector2D>) -> bool
{
    // ImGuiContext& g = *GImGui;
    ...
    let hs_for_central_nodes = f32::min(g.font_size * 1.5, f32::max(g.font_size * 0.5, parent_smaller_axis / 8.0));
    ...
    if dir == Direction::None { *out_r = Rect::from((c.x - hs_w, c.y - hs_w, c.x + hs_w, c.y + hs_w));         }
    else if dir == Direction::Up { *out_r = Rect::from((c.x - hs_w, c.y - off.y - hs_h, c.x + hs_w, c.y - off.y + hs_h)); }
    else if dir == Direction::Down { *out_r = Rect::from((c.x - hs_w, c.y + off.y - hs_h, c.x + hs_w, c.y + off.y + hs_h)); }
    else if dir == Direction::Left { *out_r = Rect::from((c.x - off.x - hs_h, c.y - hs_w, c.x - off.x + hs_h, c.y + hs_w)); }
    else if dir == Direction::Right { *out_r = Rect::from((c.x + off.x - hs_h, c.y - hs_w, c.x + off.x + hs_h, c.y + hs_w)); }
