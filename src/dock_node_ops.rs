...
use {ImGuiDataAuthority_Auto, ImGuiDataAuthority_Window};
...
use ImGuiDockNodeState_HostWindowHiddenBecauseSingleWindow;
...
use {SetWindowPos, SetWindowSize};
use End;
...
use crate::window::window_flags::ImGuiWindowFlags_UnsavedDocument;
...
pub unsafe fn DockNodeAddWindow(node:*mut ImGuiDockNode, window: *mut ImGuiWindow, add_to_tab_bar: bool)
{
    ...
    if add_to_tab_bar
    {
        if node.TabBar == null_mut()
        {
            ...
            {
                TabBarAddTab(node.TabBar, crate::tab_item_flags::ImGuiTabItemFlags_None, node.Windows[n]);
            }
        }
        TabBarAddTab(node.TabBar, crate::tab_item_flags::ImGuiTabItemFlags_Unsorted, window);
    }
    ...
    if node.HostWindow {
        crate::window::render::UpdateWindowParentAndRootLinks(window, window.Flags | crate::window::window_flags::ImGuiWindowFlags_ChildWindow, node.HostWindow);
    }
}

pub unsafe fn DockNodeRemoveWindow(node:*mut ImGuiDockNode, window: *mut ImGuiWindow, save_dock_id: crate::type_defs::ImGuiID)
{
    ...
    window.Flags &= !crate::window::window_flags::ImGuiWindowFlags_ChildWindow;
    if (window.ParentWindow) {
        window.Parentwindow.DC.ChildWindows.find_erase(window);
    }
    crate::window::render::UpdateWindowParentAndRootLinks(window, window.Flags, null_mut()); // Update immediately
...
pub unsafe fn DockNodeMoveChildNodes(dst_node:*mut ImGuiDockNode, src_node:*mut ImGuiDockNode)
{
    ...
    src_node.ChildNodes[0] = null_mut();
    src_node.ChildNodes[1]= null_mut();
}
...
pub fn DockNodeFindWindowByID(node:*mut ImGuiDockNode, id: ImGuiID) -> *mut ImGuiWindow
{
    // IM_ASSERT(id != 0);
    // for (let n: c_int = 0; n < node.Windows.len(); n++)
    for n in 0 .. node.Windows.len()
...
pub unsafe fn DockNodeUpdateFlagsAndCollapse(node:*mut ImGuiDockNode)
{
    ...
    if (node.WantHiddenTabBarToggle && is_not_null(node.VisibleWindow) && (node.Visiblewindow.WindowClass.DockNodeFlagsOverrideSet & ImGuiDockNodeFlags_HiddenTabBar)) {
...
pub unsafe fn DockNodeUpdateVisibleFlag(node: *mut ImGuiDockNode) {
    // Update visibility flag
    let mut is_visible: bool = if node.ParentNode == null_mut() { node.IsDockSpace() } else { node.IsCentralNode() };
...
pub unsafe fn DockNodeStartMouseMovingWindow(node:*mut ImGuiDockNode, window: *mut ImGuiWindow)
{
    ...
    crate::mouse_ops::StartMouseMovingWindow(window);
...
pub unsafe fn DockNodeUpdate(node:*mut ImGuiDockNode)
{
    ...
    if want_to_hide_host_window
    {
        if node.Windows.len() == 1
        {
            ...
            if is_not_null(node.HostWindow) && g.NavWindow == node.HostWindow {
                crate::window::focus::FocusWindow(single_window);
            }
            if node.HostWindow
            {
    ...
    else
    {
        ...
        if node.IsRootNode() && node.IsVisible
        {
            ...
            if node.AuthorityForPos == ImGuiDataAuthority_Window && is_not_null(ref_window) {
                SetNextWindowPos(&ref_window.Pos, 0, &Default::default());
            }
            ...
            window_flags: crate::window::window_flags::ImGuiWindowFlags = crate::window::window_flags::ImGuiWindowFlags_NoScrollbar | crate::window::window_flags::ImGuiWindowFlags_NoScrollWithMouse | crate::window::window_flags::ImGuiWindowFlags_DockNodeHost;
            window_flags |= crate::window::window_flags::ImGuiWindowFlags_NoFocusOnAppearing;
            window_flags |= crate::window::window_flags::ImGuiWindowFlags_NoSavedSettings | crate::window::window_flags::ImGuiWindowFlags_NoNavFocus | crate::window::window_flags::ImGuiWindowFlags_NoCollapse;
            window_flags |= crate::window::window_flags::ImGuiWindowFlags_NoTitleBar;

            SetNextWindowBgAlpha(0.0); // Don't set ImGuiWindowFlags_NoBackground because it disables borders
            PushStyleVar(crate::style_var::ImGuiStyleVar_WindowPadding, crate::vec2::ImVec2::new(0, 0));
    ...
    if (central_node_hole)
        if (*const ImGuiPayload payload = crate::drag_drop_ops::GetDragDropPayload())
    ...
    if (central_node_hole_register_hit_test_hole)
    {
        ...
        let mut root_rect: crate::rect::ImRect = crate::rect::ImRect::new(root_node.Pos, root_node.Pos + root_node.Size);
        let mut hole_rect: crate::rect::ImRect = crate::rect::ImRect::new(central_node.Pos, central_node.Pos + central_node.Size);
        if (hole_rect.Min.x > root_rect.Min.x) { hole_rect.Min.x += crate::constants::WINDOWS_HOVER_PADDING; }
        if (hole_rect.Max.x < root_rect.Max.x) { hole_rect.Max.x -= crate::constants::WINDOWS_HOVER_PADDING; }
        if (hole_rect.Min.y > root_rect.Min.y) { hole_rect.Min.y += crate::constants::WINDOWS_HOVER_PADDING; }
        if (hole_rect.Max.y < root_rect.Max.y) { hole_rect.Max.y -= crate::constants::WINDOWS_HOVER_PADDING; }
    ...
    if (host_window && node.IsEmpty() && node.IsVisible)
    {
        ...
        node.LastBgColor = if node_flags & ImGuiDockNodeFlags_PassthruCentralNode { 0} else { crate::style_ops::GetColorU32(crate::color::ImGuiCol_DockingEmptyBg)};
    ...
    if (render_dockspace_bg && node.IsVisible)
    {
        host_window.DrawList.ChannelsSetCurrent(DOCKING_HOST_DRAW_CHANNEL_BG);
        if (central_node_hole)
            crate::render_ops::RenderRectFilledWithHole(host_window.DrawList, node.Rect(), central_node.Rect(), crate::style_ops::GetColorU32(crate::color::ImGuiCol_WindowBg), 0.0);
        else
            host_window.DrawList.AddRectFilled(node.Pos, node.Pos + node.Size, crate::style_ops::GetColorU32(crate::color::ImGuiCol_WindowBg), 0.0);
    }
    ...
    if (host_window && node.IsVisible)
        if (node.IsRootNode() && (g.MovingWindow == null_mut() || g.Movingwindow.RootWindowDockTree != host_window))
            crate::imgui_cpp::BeginDockableDragDropTarget(host_window);
    ...
    if (host_window)
    {
        ...
        if (node.IsRootNode())
            crate::window::render::RenderWindowOuterBorders(host_window);
    }
...
static DockNodeUpdateWindowMenu: crate::type_defs::ImGuiID:*mut ImGuiDockNode, ImGuiTabBar* tab_bar)
{
...
pub unsafe fn DockNodeBeginAmendTabBar(node:*mut ImGuiDockNode) -> bool
{
    ...
    crate::id_ops::PushOverrideID(node.ID);
...
{
    let g = GImGui; // ImGuiContext& g = *GImGui;
    crate::style::ImGuiStyle & style = g.Style;

    let node_was_active: bool = (node.LastFrameActive + 1 == g.FrameCount);
    let closed_all: bool = node.WantCloseAll && node_was_active;
    let mut closed_one: crate::type_defs::ImGuiID =  node.WantCloseTabId && node_was_active;
    node.WantCloseAll = false;
    node.WantCloseTabId = 0;
    ...
    if (!node.IsDockSpace())
    {
        ...
        host_window.DC.NavLayerCurrent = crate::nav_layer::ImGuiNavLayer_Menu;
    }
    ...
    let mut focus_tab_id: crate::type_defs::ImGuiID =  0;
    node.IsFocused = is_focused;

    const ImGuiDockNodeFlags node_flags = node.MergedFlags;
    let has_window_menu_button: bool = (node_flags & ImGuiDockNodeFlags_NoWindowMenuButton) == 0 && (style.WindowMenuButtonPosition != crate::direction::ImGuiDir_None);

    // In a dock node, the Collapse Button turns into the Window Menu button.
    // FIXME-DOCK FIXME-OPT: Could we recycle popups id across multiple dock nodes?
    if (has_window_menu_button && crate::popup_ops::IsPopupOpen("#WindowMenu"))
    {
        if (let mut tab_id: crate::type_defs::ImGuiID =  DockNodeUpdateWindowMenu(node, tab_bar))
    ...
    window_menu_button_pos: crate::vec2::ImVec2;
    close_button_pos: crate::vec2::ImVec2;
    ...
    for (let window_n: c_int = 0; window_n < node.Windows.len(); window_n++)
    {
        ...
        if (TabBarFindTabByID(tab_bar, window.TabId) == null_mut())
            TabBarAddTab(tab_bar, crate::tab_item_flags::ImGuiTabItemFlags_Unsorted, window);
    }

    // Title bar
    if (is_focused)
        node.LastFrameFocused = g.FrameCount;
    title_bar_col: u32 = crate::style_ops::GetColorU32(host_window.Collapsed ? crate::color::ImGuiCol_TitleBgCollapsed: is_focused ? crate::color::ImGuiCol_TitleBgActive: ImGuiCol_TitleBg);
    rounding_flags: crate::draw_flags::ImDrawFlags = crate::render_ops::CalcRoundingFlagsForRectInRect(title_bar_rect, host_window.Rect(), crate::constants::DOCKING_SPLITTER_SIZE);
    ...
    if (has_window_menu_button)
    {
        if (CollapseButton(host_window.GetID("#COLLAPSE"), window_menu_button_pos, node)) // == DockNodeGetWindowMenuButtonId(node)
            crate::popup_ops::OpenPopup("#WindowMenu");
        if (crate::item_ops::IsItemActive())
    ...
    for (let tab_n: c_int = tab_bar->Tabs.Size - 1; tab_n >= 0 && (tab_bar->Tabs[tab_n].Flags & ImGuiTabItemFlags_Unsorted); tab_n--)
    {
        ...
        tab_bar->Tabs[tab_n].Flags &= !crate::tab_item_flags::ImGuiTabItemFlags_Unsorted;
    ...
    if (tab_bar->Tabs.Size > tabs_unsorted_start)
    {
        ...
        if (tab_bar->Tabs.Size > tabs_unsorted_start + 1)
            crate::utils::ImQsort(tab_bar->Tabs.Data + tabs_unsorted_start, tab_bar->Tabs.Size - tabs_unsorted_start, sizeof(crate::tab_item::ImGuiTabItem), TabItemComparerByDockOrder);
    }
    ...
    // Begin tab bar
    crate::tab_bar_flags::ImGuiTabBarFlags
    tab_bar_flags = crate::tab_bar_flags::ImGuiTabBarFlags_Reorderable | crate::tab_bar_flags::ImGuiTabBarFlags_AutoSelectNewTabs; // | ImGuiTabBarFlags_NoTabListScrollingButtons);
    tab_bar_flags |= crate::tab_bar_flags::ImGuiTabBarFlags_SaveSettings | crate::tab_bar_flags::ImGuiTabBarFlags_DockNode;
    if (!host_window.Collapsed && is_focused)
        tab_bar_flags |= crate::tab_bar_flags::ImGuiTabBarFlags_IsFocused;
    BeginTabBarEx(tab_bar, tab_bar_rect, tab_bar_flags, node);
    //host_window.DrawList.AddRect(tab_bar_rect.Min, tab_bar_rect.Max, IM_COL32(255,0,255,255));

    // Backup style colors
    backup_style_cols: crate::vec4::ImVec4[crate::window::window_dock_style_color::ImGuiWindowDockStyleCol_COUNT];
    for (let color_n: c_int = 0; color_n < ImGuiWindowDockStyleCol_COUNT; color_n++)
        backup_style_cols[color_n] = g.Style.Colors[crate::window::window_dock_style_colors::GWindowDockStyleColors[color_n]];
    ...
    {
        let mut window: *mut ImGuiWindow =  node.Windows[window_n];
        if ((closed_all || closed_one == window.TabId) && window.HasCloseButton && !(window.Flags & crate::window::window_flags::ImGuiWindowFlags_UnsavedDocument))
            continue;
        if (window.LastFrameActive + 1 >= g.FrameCount || !node_was_active)
        {
            crate::tab_item_flags::ImGuiTabItemFlags
            tab_item_flags = 0;
            tab_item_flags |= window.WindowClass.TabItemFlagsOverrideSet;
            if (window.Flags & crate::window::window_flags::ImGuiWindowFlags_UnsavedDocument)
                tab_item_flags |= crate::tab_item_flags::ImGuiTabItemFlags_UnsavedDocument;
            if (tab_bar.Flags & crate::tab_bar_flags::ImGuiTabBarFlags_NoCloseWithMiddleMouseButton)
                tab_item_flags |= crate::tab_item_flags::ImGuiTabItemFlags_NoCloseWithMiddleMouseButton;

            // Apply stored style overrides for the window
            for (let color_n: c_int = 0; color_n < ImGuiWindowDockStyleCol_COUNT; color_n++)
                g.Style.Colors[crate::window::window_dock_style_colors::GWindowDockStyleColors[color_n]] = crate::color_ops::ColorConvertU32ToFloat4(window.DockStyle.Colors[color_n]);
            ...
            if (g.NavWindow && g.NavWindow.RootWindow == window && (window.DC.NavLayersActiveMask & (1 << crate::nav_layer::ImGuiNavLayer_Menu)) == 0)
    ...
    for (let color_n: c_int = 0; color_n < ImGuiWindowDockStyleCol_COUNT; color_n++)
        g.Style.Colors[crate::window::window_dock_style_colors::GWindowDockStyleColors[color_n]] = backup_style_cols[color_n];
    ...
    if (close_button_is_visible)
    {
        ...
        {
            crate::item_ops::PushItemFlag(crate::item_flags::ImGuiItemFlags_Disabled, true);
            crate::style_ops::PushStyleColor(crate::color::ImGuiCol_Text, style.Colors[crate::color::ImGuiCol_Text] * crate::vec4::ImVec4(1.0, 1.0, 1.0, 0.40f32));
        }
        ...
        if (!close_button_is_enabled)
        {
            crate::style_ops::PopStyleColor();
            crate::item_ops::PopItemFlag();
        }
    }

    // When clicking on the title bar outside of tabs, we still focus the selected tab for that node
    // FIXME: TabItem use AllowItemOverlap so we manually perform a more specific test for now (hovered || held)
    let mut title_bar_id: crate::type_defs::ImGuiID =  host_window.GetID("#TITLEBAR");
    if (g.HoveredId == 0 || g.HoveredId == title_bar_id || g.ActiveId == title_bar_id)
    {
        ...
        if (held)
        {
            if (crate::input_ops::IsMouseClicked(0))
                focus_tab_id = tab_bar->SelectedTabId;

            // Forward moving request to selected window
            if (crate::tab_item::ImGuiTabItem * tab = TabBarFindTabByID(tab_bar, tab_bar->SelectedTabId))
                crate::mouse_ops::StartMouseMovingWindowOrNode(tab->Window ? tab->Window : node.HostWindow, node, false);
        }
    }
    ...
    if (focus_tab_id != 0)
        if (crate::tab_item::ImGuiTabItem * tab = TabBarFindTabByID(tab_bar, focus_tab_id))
            if (tab->Window)
            {
                crate::window::focus::FocusWindow(tab->Window);
                crate::nav_ops::NavInitWindow(tab->Window, false);
            }
    ...
    if (!node.IsDockSpace())
    {
        host_window.DC.NavLayerCurrent = crate::nav_layer::ImGuiNavLayer_Main;
...
pub unsafe fn DockNodeIsDropAllowedOne(payload: *mut ImGuiWindow, host_window: *mut ImGuiWindow) -> bool
{
    ...
    crate::window::window_class::ImGuiWindowClass * host_class = host_window.DockNodeAsHost ? &host_window.DockNodeAsHost->WindowClass : &host_window.WindowClass;
    ...
    for (let i: c_int = g.OpenPopupStack.len() - 1; i >= 0; i--)
        if (let mut popup_window: *mut ImGuiWindow =  g.OpenPopupStack[i].Window)
            if (crate::window::find::IsWindowWithinBeginStackOf(payload, popup_window))   // Payload is created from within a popup begin stack.
                return false;
...
pub unsafe fn DockNodeIsDropAllowed(host_window: *mut ImGuiWindow, root_payload: *mut ImGuiWindow) -> bool
{
    ...
    let payload_count: c_int = root_payload->DockNodeAsHost ? root_payload->DockNodeAsHost-> std::slice::Windows.len() : 1;
    for (let payload_n: c_int = 0; payload_n < payload_count; payload_n++)
    {
        let mut payload: *mut ImGuiWindow =  root_payload->DockNodeAsHost ? root_payload->DockNodeAsHost-> std::slice::Windows[payload_n] : root_payload;
...
pub unsafe fn DockNodeCalcTabBarLayout(*const ImGuiDockNode node, crate::rect::ImRect* out_title_rect, crate::rect::ImRect* out_tab_bar_rect, out_window_menu_button_pos: *mut crate::vec2::ImVec2, out_close_button_pos: *mut crate::vec2::ImVec2)
{
    let g = GImGui; // ImGuiContext& g = *GImGui;
    crate::style::ImGuiStyle & style = g.Style;

    let r: crate::rect::ImRect =  crate::rect::ImRect(node.Pos.x, node.Pos.y, node.Pos.x + node.Size.x, node.Pos.y + g.FontSize + g.Style.FramePadding.y * 2.00f32);
    ...
    let window_menu_button_pos: crate::vec2::ImVec2 = r.Min;
    r.Min.x += style.FramePadding.x;
    r.Max.x -= style.FramePadding.x;
    if (node.HasCloseButton)
    {
        r.Max.x -= button_sz;
        if (out_close_button_pos) *out_close_button_pos = crate::vec2::ImVec2::new(r.Max.x - style.FramePadding.x, r.Min.y);
    }
    if (node.HasWindowMenuButton && style.WindowMenuButtonPosition == crate::direction::ImGuiDir_Left)
    {
        r.Min.x += button_sz + style.ItemInnerSpacing.x;
    }
    else if (node.HasWindowMenuButton && style.WindowMenuButtonPosition == crate::direction::ImGuiDir_Right)
    {
        r.Max.x -= button_sz + style.FramePadding.x;
        window_menu_button_pos = crate::vec2::ImVec2::new(r.Max.x, r.Min.y);
    }
    if (out_tab_bar_rect) { *out_tab_bar_rect = r; }
    if (out_window_menu_button_pos) { *out_window_menu_button_pos = window_menu_button_pos; }
}

pub unsafe fn DockNodeCalcSplitRects(crate::vec2::ImVec2& pos_old, crate::vec2::ImVec2& size_old, crate::vec2::ImVec2& pos_new, crate::vec2::ImVec2& size_new, dir: crate::direction::ImGuiDir, size_new_desired: crate::vec2::ImVec2)
{
    let g = GImGui; // ImGuiContext& g = *GImGui;
    let dock_spacing: c_float =  g.Style.ItemInnerSpacing.x;
    const axis: crate::axis::ImGuiAxis = if dir == crate::direction::ImGuiDir_Left || dir == crate::direction::ImGuiDir_Right { crate::axis::ImGuiAxis_X } else { crate::axis::ImGuiAxis_Y };
    ...
    // Position each node
    if (dir == crate::direction::ImGuiDir_Right || dir == crate::direction::ImGuiDir_Down)
    {
        pos_new[axis] = pos_old[axis] + size_old[axis] + dock_spacing;
    }
    else if (dir == crate::direction::ImGuiDir_Left || dir == crate::direction::ImGuiDir_Up)
    {
...
pub unsafe fn DockNodeCalcDropRectsAndTestMousePos(parent: &crate::rect::ImRect, dir: crate::direction::ImGuiDir, out_r: &mut crate::rect::ImRect, outer_docking: bool, test_mouse_pos: *mut crate::vec2::ImVec2) -> bool
{
    let g = GImGui; // ImGuiContext& g = *GImGui;

    let parent_smaller_axis: c_float =  crate::math_ops::ImMin(parent.GetWidth(), parent.GetHeight());
    let hs_for_central_nodes: c_float =  crate::math_ops::ImMin(g.FontSize * 1.5f32, crate::math_ops::ImMax(g.FontSize * 0.5, parent_smaller_axis / 8.00f32));
    let mut hs_w: c_float = 0.0; // Half-size, longer axis
    let mut hs_h: c_float = 0.0; // Half-size, smaller axis
    off: crate::vec2::ImVec2; // Distance from edge or center
    if (outer_docking)
    {
        ...
        off = crate::vec2::ImVec2::new(ImFloor(parent.GetWidth() * 0.5 - hs_h), ImFloor(parent.GetHeight() * 0.5 - hs_h));
    }
    else
    {
        hs_w = ImFloor(hs_for_central_nodes);
        hs_h = ImFloor(hs_for_central_nodes * 0.900f32);
        off = crate::vec2::ImVec2::new(ImFloor(hs_w * 2.400f32), ImFloor(hs_w * 2.400f32));
    }

    let c: crate::vec2::ImVec2 = ImFloor(parent.GetCenter());
    if      (dir == crate::direction::ImGuiDir_None)  { out_r = crate::rect::ImRect(c.x - hs_w, c.y - hs_w, c.x + hs_w, c.y + hs_w);         }
    else if (dir == crate::direction::ImGuiDir_Up)    { out_r = crate::rect::ImRect(c.x - hs_w, c.y - off.y - hs_h, c.x + hs_w, c.y - off.y + hs_h); }
    else if (dir == crate::direction::ImGuiDir_Down)  { out_r = crate::rect::ImRect(c.x - hs_w, c.y + off.y - hs_h, c.x + hs_w, c.y + off.y + hs_h); }
    else if (dir == crate::direction::ImGuiDir_Left)  { out_r = crate::rect::ImRect(c.x - off.x - hs_h, c.y - hs_w, c.x - off.x + hs_h, c.y + hs_w); }
    else if (dir == crate::direction::ImGuiDir_Right) { out_r = crate::rect::ImRect(c.x + off.x - hs_h, c.y - hs_w, c.x + off.x + hs_h, c.y + hs_w); }

    if (test_mouse_pos == null_mut())
        return false;

    let hit_r: crate::rect::ImRect =  out_r;
    if (!outer_docking)
    {
        // Custom hit testing for the 5-way selection, designed to reduce flickering when moving diagonally between sides
        hit_r.Expand(ImFloor(hs_w * 0.300f32));
        let mouse_delta: crate::vec2::ImVec2 = (*test_mouse_pos - c);
        ...
        if (mouse_delta_len2 < r_threshold_center * r_threshold_center)
            return (dir == crate::direction::ImGuiDir_None);
        if (mouse_delta_len2 < r_threshold_sides * r_threshold_sides)
            return (dir == crate::nav_ops::ImGetDirQuadrantFromDelta(mouse_delta.x, mouse_delta.y));
    }
...
pub unsafe fn DockNodePreviewDockSetup(host_window: *mut ImGuiWindow, host_node:*mut ImGuiDockNode, payload_window: *mut ImGuiWindow, payload_node:*mut ImGuiDockNode, crate::dock_preview_data::ImGuiDockPreviewData* data, is_explicit_target: bool, is_outer_docking: bool)
{
    let g = GImGui; // ImGuiContext& g = *GImGui;
    ...
    data.FutureNode.HasWindowMenuButton = host_node ? true : ((host_window.Flags & crate::window::window_flags::ImGuiWindowFlags_NoCollapse) == 0);
    ...
    data.SplitDir = crate::direction::ImGuiDir_None;
    data.IsSplitDirExplicit = false;
    if (!host_window.Collapsed)
        for (let dir: c_int = ImGuiDir_None; dir < ImGuiDir_COUNT; dir++)
        {
            if (dir == crate::direction::ImGuiDir_None && !data.IsCenterAvailable)
                continue;
            if (dir != crate::direction::ImGuiDir_None && !data.IsSidesAvailable)
                continue;
            if (DockNodeCalcDropRectsAndTestMousePos(data.FutureNode.Rect(), (crate::direction::ImGuiDir)dir, data.DropRectsDraw[dir1], is_outer_docking, &g.IO.MousePos))
            {
                data.SplitDir = (crate::direction::ImGuiDir)dir;
                data.IsSplitDirExplicit = true;
            }
        }

    // When docking without holding Shift, we only allow and preview docking when hovering over a drop rect or over the title bar
    data.IsDropAllowed = (data.SplitDir != crate::direction::ImGuiDir_None) || (data.IsCenterAvailable);
    if (!is_explicit_target && !data.IsSplitDirExplicit && !g.IO.ConfigDockingWithShift)
        data.IsDropAllowed = false;

    // Calculate split area
    data.SplitRatio = 0.0;
    if (data.SplitDir != crate::direction::ImGuiDir_None)
    {
        split_dir: crate::direction::ImGuiDir = data.SplitDir;
        split_axis: crate::axis::ImGuiAxis = if split_dir == crate::direction::ImGuiDir_Left || split_dir == crate::direction::ImGuiDir_Right { crate::axis::ImGuiAxis_X } else { crate::axis::ImGuiAxis_Y };
        pos_new: crate::vec2::ImVec2, pos_old = data.FutureNode.Pos;
        size_new: crate::vec2::ImVec2, size_old = data.FutureNode.Size;
        ...
        data.SplitRatio = if split_dir == crate::direction::ImGuiDir_Right || split_dir == crate::direction::ImGuiDir_Down { (1.0 - split_ratio)} else { (split_ratio)};
    }
}
...
pub unsafe fn DockNodeTreeSplit(ctx: *mut crate::context::ImGuiContext, parent_node:*mut ImGuiDockNode, split_axis: crate::axis::ImGuiAxis, split_inheritor_child_idx: c_int, split_ratio: c_float, new_node:*mut ImGuiDockNode)
{
    let g = GImGui; // ImGuiContext& g = *GImGui;
    // IM_ASSERT(split_axis != ImGuiAxis_None);

    child_0:*mut ImGuiDockNode = if new_node && split_inheritor_child_idx != 0 { new_node} else { crate::dock_context_ops::DockContextAddNode(ctx, 0)};
    child_0->ParentNode = parent_node;

    child_1:*mut ImGuiDockNode = if new_node && split_inheritor_child_idx != 1 { new_node} else { crate::dock_context_ops::DockContextAddNode(ctx, 0)};
    ...
    let size_avail: c_float =  (parent_node.Size[split_axis] - crate::constants::DOCKING_SPLITTER_SIZE);
    size_avail = crate::math_ops::ImMax(size_avail, g.Style.WindowMinSize[split_axis] * 2.00f32);
    ...
    crate::imgui_cpp::DockSettingsRenameNodeReferences(parent_node.ID, parent_node.ChildNodes[split_inheritor_child_idx]->ID);
...
pub unsafe fn DockNodeTreeMerge(ctx: *mut crate::context::ImGuiContext, parent_node:*mut ImGuiDockNode, merge_lead_child:*mut ImGuiDockNode)
{
    ...
    if ((child_0 && child_0-> std::slice::Windows.len() > 0) || (child_1 && child_1-> std::slice::Windows.len() > 0))
    {
    ...
    let backup_last_explicit_size: crate::vec2::ImVec2 = parent_node.SizeRef;
    DockNodeMoveChildNodes(parent_node, merge_lead_child);
    if (child_0)
    {
        DockNodeMoveWindows(parent_node, child_0); // Generally only 1 of the 2 child node will have windows
        crate::imgui_cpp::DockSettingsRenameNodeReferences(child_0->ID, parent_node.ID);
    }
    if (child_1)
    {
        DockNodeMoveWindows(parent_node, child_1);
        crate::imgui_cpp::DockSettingsRenameNodeReferences(child_1->ID, parent_node.ID);
    }
...
pub unsafe fn DockNodeTreeUpdatePosSize(node:*mut ImGuiDockNode, pos: crate::vec2::ImVec2, size: crate::vec2::ImVec2, only_write_to_single_node:*mut ImGuiDockNode)
{
    ...
    let child_0_pos: crate::vec2::ImVec2 = pos, child_1_pos = pos;
    let child_0_size: crate::vec2::ImVec2 = size, child_1_size = size;
    ...
    {
        let g = GImGui; // ImGuiContext& g = *GImGui;
        let spacing: c_float = crate::constants::DOCKING_SPLITTER_SIZE;
        const axis: crate::axis::ImGuiAxis = node.SplitAxis;
        let size_avail: c_float =  crate::math_ops::ImMax(size[axis] - spacing, 0.0);

        // Size allocation policy
        // 1) The first 0..WindowMinSize[axis]*2 are allocated evenly to both windows.
        let size_min_each: c_float =  ImFloor(crate::math_ops::ImMin(size_avail, g.Style.WindowMinSize[axis] * 2.00f32) * 0.5);
        ...
        {
            child_0_size[axis] = child_0->SizeRef[axis] = crate::math_ops::ImMin(size_avail - 1.0, child_0->Size[axis]);
            child_1_size[axis] = child_1->SizeRef[axis] = (size_avail - child_0_size[axis]);
            // IM_ASSERT(child_0->SizeRef[axis] > 0.0 && child_1->SizeRef[axis] > 0.0);
        }
        else if (child_1->WantLockSizeOnce && !child_0->WantLockSizeOnce)
        {
            child_1_size[axis] = child_1->SizeRef[axis] = crate::math_ops::ImMin(size_avail - 1.0, child_1->Size[axis]);
        ...
        {
            child_0_size[axis] = crate::math_ops::ImMin(size_avail - size_min_each, child_0->SizeRef[axis]);
            child_1_size[axis] = (size_avail - child_0_size[axis]);
        }
        else if (child_1->SizeRef[axis] != 0.0 && child_0->HasCentralNodeChild)
        {
            child_1_size[axis] = crate::math_ops::ImMin(size_avail - size_min_each, child_1->SizeRef[axis]);
        ...
        else
        {
            ...
            child_0_size[axis] = crate::math_ops::ImMax(size_min_each, ImFloor(size_avail * split_ratio + 0.5));
            child_1_size[axis] = (size_avail - child_0_size[axis]);
        }
...
pub unsafe fn DockNodeTreeUpdateSplitterFindTouchingNode(node:*mut ImGuiDockNode, axis: crate::axis::ImGuiAxis, side: c_int, Vec<ImGuiDockNode*> * touching_nodes)
{
    if (node.IsLeafNode())
    {
...
pub unsafe fn DockNodeTreeUpdateSplitter(node:*mut ImGuiDockNode)
{
    ...
    if (child_0->IsVisible && child_1->IsVisible)
    {
        ...
        let mut bb: crate::rect::ImRect = crate::rect::ImRect::default();
        bb.Min = child_0->Pos;
        bb.Max = child_1->Pos;
        ...
        const ImGuiDockNodeFlags no_resize_axis_flag = if axis == crate::axis::ImGuiAxis_X { ImGuiDockNodeFlags_NoResizeX } else { ImGuiDockNodeFlags_NoResizeY };
        if ((merged_flags & ImGuiDockNodeFlags_NoResize) || (merged_flags & no_resize_axis_flag))
        {
            let mut window = g.CurrentWindow;
            window.DrawList.AddRectFilled(bb.Min, bb.Max, crate::style_ops::GetColorU32(crate::color::ImGuiCol_Separator), g.Style.FrameRounding);
        }
        else
        {
            //bb.Min[axis] += 1; // Display a little inward so highlight doesn't connect with nearby tabs on the neighbor node.
            //bb.Max[axis] -= 1;
            crate::id_ops::PushID(node.ID);
            ...
            let mut splitter_id: crate::type_defs::ImGuiID =  crate::id_ops::GetID("##Splitter");
            if (g.ActiveId == splitter_id) // Only process when splitter is active
            {
                ...
                for (let touching_node_n: c_int = 0; touching_node_n < touching_nodes[0].Size; touching_node_n++)
                    resize_limits[0] = crate::math_ops::ImMax(resize_limits[0], touching_nodes[0][touching_node_n]->Rect().Min[axis] + min_size);
                for (let touching_node_n: c_int = 0; touching_node_n < touching_nodes[1].Size; touching_node_n++)
                    resize_limits[1] = crate::math_ops::ImMin(resize_limits[1], touching_nodes[1][touching_node_n]->Rect().Max[axis] - min_size);
            ...
            bg_col: u32 = crate::style_ops::GetColorU32(crate::color::ImGuiCol_WindowBg);
            if (SplitterBehavior(bb, crate::id_ops::GetID("##Splitter"), axis, &cur_size_0, &cur_size_1, min_size_0, min_size_1, crate::constants::WINDOWS_HOVER_PADDING, crate::constants::WINDOWS_RESIZE_FROM_EDGES_FEEDBACK_TIMER, bg_col))
            {
