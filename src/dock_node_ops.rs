...
pub fn DockNodeIsInHierarchy(mut node: *mut ImGuiDockNode, parent: *mut ImGuiDockNode) -> bool {
    while node {
        if node == parent {
            return true;
        }
...
pub fn DockNodeGetDepth(mut node: *const ImGuiDockNode) -> c_int {
...
pub unsafe fn DockNodeGetWindowMenuButtonId(node: *const ImGuiDockNode) -> ImGuiID {
    return ImHashStr(str_to_const_c_char_ptr("#COLLAPSE"), 0, node.ID as u32);
}
...
// c_int DockNodeGetTabOrder(window: *mut ImGuiWindow)
...
pub fn DockNodeHideWindowDuringHostWindowCreation(window: *mut ImGuiWindow)
{
    window.Hidden = true;
    window.HiddenFramesCanSkipItems = if window.Active { 1 } else { 2 };
}

// static c_void DockNodeAddWindow(node: *mut ImGuiDockNode, window: *mut ImGuiWindow, add_to_tab_bar: bool)
...
// static c_void DockNodeRemoveWindow(node: *mut ImGuiDockNode, window: *mut ImGuiWindow, save_dock_id: ImGuiID)
pub unsafe fn DockNodeRemoveWindow(node: *mut ImGuiDockNode, window: *mut ImGuiWindow, save_dock_id: ImGuiID)
{
    ...
    if node.TabBar
    {
        ...
        let tab_count_threshold_for_tab_bar: c_int = if node.IsCentralNode() { 1 } else {2};
    ...
    if node.Windows.len() == 1 && !node.IsCentralNode() && node.HostWindow.is_null() == false
    {
...
// static c_void DockNodeMoveChildNodes(dst_node: *mut ImGuiDockNode, src_node: *mut ImGuiDockNode)
pub fn DockNodeMoveChildNodes(dst_node: *mut ImGuiDockNode, src_node: *mut ImGuiDockNode) {
...
// static c_void DockNodeMoveWindows(dst_node: *mut ImGuiDockNode, src_node: *mut ImGuiDockNode)
pub unsafe fn DockNodeMoveWindows(dst_node: *mut ImGuiDockNode, src_node: *mut ImGuiDockNode)
{
    // Insert tabs in the same orders as currently ordered (node.Windows isn't ordered)
    // IM_ASSERT(src_node && dst_node && dst_node != src_node);
    let mut src_tab_bar: *mut ImGuiTabBar =  src_node.TabBar;
    ...
    if !move_tab_bar && src_node.TabBar.is_null() == false
    {
        if dst_node.TabBar {
...
// static c_void DockNodeApplyPosSizeToWindows(node: *mut ImGuiDockNode)
pub fn DockNodeApplyPosSizeToWindows(node: *mut ImGuiDockNode)
{
    // for (let n: c_int = 0; n < node.Windows.len(); n++)
    for n in 0 .. node.Windows.len()
    {
...
// static c_void DockNodeHideHostWindow(node: *mut ImGuiDockNode)
pub fn DockNodeHideHostWindow(node: *mut ImGuiDockNode) {
    if node.HostWindow {
...
// static c_void DockNodeFindInfo(node: *mut ImGuiDockNode, info: *mut ImGuiDockTreeInfo)
pub fn DockNodeFindInfo(node: *mut ImGuiDockNode, info: *mut ImGuiDockTreeInfo) {
...
// static ImGuiWindow* DockNodeFindWindowByID(node: *mut ImGuiDockNode, id: ImGuiID)
...
// static c_void DockNodeUpdateFlagsAndCollapse(node: *mut ImGuiDockNode)
pub unsafe fn  DockNodeUpdateFlagsAndCollapse(node: *mut ImGuiDockNode)
{
    ...
    let mut node_flags: ImGuiDockNodeFlags = node.MergedFlags;
    ...
    if node.WantHiddenTabBarToggle && node.VisibleWindow.is_null() == false && flag_set(node.Visiblewindow.WindowClass.DockNodeFlagsOverrideSet, ImGuiDockNodeFlags_HiddenTabBar) {
...
// static c_void DockNodeUpdateHasCentralNodeChild(node: *mut ImGuiDockNode)
pub fn DockNodeUpdateHasCentralNodeChild(node: *mut ImGuiDockNode) {
    node.HasCentralNodeChild = false;
    if node.ChildNodes[0] {
...
// static c_void DockNodeUpdateVisibleFlag(node: *mut ImGuiDockNode)
pub fn DockNodeUpdateVisibleFlag(node: *mut ImGuiDockNode)
{
    // Update visibility flag
    ...
    is_visible |= (node.ChildNodes[0].is_null() == false && node.ChildNodes[0].IsVisible);
    is_visible |= (node.ChildNodes[1].is_null() == false && node.ChildNodes[1].IsVisible);
    node.IsVisible = is_visible;
}

// static c_void DockNodeStartMouseMovingWindow(node: *mut ImGuiDockNode, window: *mut ImGuiWindow)
...
// static c_void DockNodeUpdateForRootNode(node: *mut ImGuiDockNode)
pub unsafe fn DockNodeUpdateForRootNode(node: *mut ImGuiDockNode)
{
    ...
    let mut first_node_with_windows: *mut ImGuiDockNode = info.FirstNodeWithWindows;
    if first_node_with_windows.is_null() == false
    {
        node.WindowClass = first_node_with_windows.Windows[0].WindowClass.clone();
        // for (let n: c_int = 1; n < first_node_with_windows.Windows.len(); n++)\
        for n in 1 .. first_node_with_windows.Windows.len()
        {
            if first_node_with_windows.Windows[n].WindowClass.DockingAllowUnclassed == false {
                node.WindowClass = first_node_with_windows.Windows[n].WindowClass.clone();
                break;
            }
        }
    }

    let mut mark_node: *mut ImGuiDockNode = node.CentralNode;
    while mark_node
    {
        mark_node.HasCentralNodeChild = true;
        mark_node = mark_node.ParentNode;
...
// static c_void DockNodeSetupHostWindow(node: *mut ImGuiDockNode, host_window: *mut ImGuiWindow)
pub fn DockNodeSetupHostWindow(node: *mut ImGuiDockNode, host_window: *mut ImGuiWindow)
{
    ...
    if node.HostWindow.is_null() == false && node.HostWindow != host_window && node.Hostwindow.DockNodeAsHost == node {
        node.Hostwindow.DockNodeAsHost = null_mut();
    }
...
// static c_void DockNodeUpdate(node: *mut ImGuiDockNode)
pub unsafe fn DockNodeUpdate(node: *mut ImGuiDockNode)
{
    ...
    if node.TabBar.is_null() == false && node.IsNoTabBar() {
        DockNodeRemoveTabBar(node);
    }
    ...
    if want_to_hide_host_window
    {
        if node.Windows.len() == 1
        {
            ...
            if node.HostWindow.is_null() == false && g.NavWindow == node.HostWindow {
    ...
    else
    {
        ...
        {
            let mut ref_window: *mut ImGuiWindow = if node.Windows.len() > 0 { node.Windows[0] } else { null_mut() };

            // Sync Pos
            if (node.AuthorityForPos == ImGuiDataAuthority_Window && ref_window.is_null() == false) {
            ...
            let mut window_label: [c_char;20] = [0;20];
            DockNodeGetHostWindowTitle(node, window_label, IM_ARRAYSIZE(window_label));
            let mut window_flags: ImGuiWindowFlags = ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_DockNodeHost;
            ...
            node.AuthorityForPos = ImGuiDataAuthority_Auto;node.AuthorityForSize = ImGuiDataAuthority_Auto;node.AuthorityForViewport = ImGuiDataAuthority_Auto;
        }
        else if node.ParentNode
        {
            node.HostWindow = node.ParentNode.HostWindow;host_window = node.ParentNode.HostWindow;
            node.AuthorityForPos = ImGuiDataAuthority_Auto;node.AuthorityForSize = ImGuiDataAuthority_Auto;node.AuthorityForViewport = ImGuiDataAuthority_Auto;
        }
    ...
    let mut central_node: *mut ImGuiDockNode = node.CentralNode;
    let central_node_hole: bool = node.IsRootNode() && host_window.is_null() == false && (node_flags & ImGuiDockNodeFlags_PassthruCentralNode) != 0 && central_node != null_mut() && central_node.IsEmpty();
    let mut central_node_hole_register_hit_test_hole: bool =  central_node_hole;
    if central_node_hole {
        let mut payload: *ImGuiPayload = GetDragDropPayload();
    ...
    if central_node_hole_register_hit_test_hole
    {
        ...
        let mut root_node: *mut ImGuiDockNode = DockNodeGetRootNode(&mut *central_node);
        let mut root_rect: ImRect = ImRect::new2(&root_node.Pos, root_node.Pos + root_node.Size);
        let mut hole_rect: ImRect = ImRect::new2(&central_node.Pos, central_node.Pos + central_node.Size);
    ...
    let render_dockspace_bg: bool = node.IsRootNode() && host_window.is_null() == false && flag_set(node_flags, ImGuiDockNodeFlags_PassthruCentralNode);
...
pub fn TabItemComparerByDockOrder(lhs: *const ImGuiTabItem, rhs: *const ImGuiTabItem) -> c_int
{
    ...
    let mut d = if lhs.Window.DockOrder == -1 {
        INT_MAX
    } else {
        lhs.Window.DockOrder
    };
    d = d - if rhs.Window.DockOrder == -1 {INT_MAX} else {rhs.Window.DockOrder};
    if d != 0 {
        return d;
    }
    return (lhs.Window.BeginOrderWithinContext - rhs.Window.BeginOrderWithinContext) as c_int;
}

// static ImGuiID DockNodeUpdateWindowMenu(node: *mut ImGuiDockNode, tab_bar: *mut ImGuiTabBar)
pub unsafe fn DockNodeUpdateWindowMenu(node: *mut ImGuiDockNode, tab_bar: *mut ImGuiTabBar) -> ImGuiID
{
...
// bool DockNodeBeginAmendTabBar(node: *mut ImGuiDockNode)
...
// static bool IsDockNodeTitleBarHighlighted(node: *mut ImGuiDockNode, root_node: *mut ImGuiDockNode, host_window: *mut ImGuiWindow)
pub unsafe fn IsDockNodeTitleBarHighlighted(node: *mut ImGuiDockNode, root_node: *mut ImGuiDockNode, host_window: *mut ImGuiWindow) -> bool {
    ...
    if g.NavWindow.is_null() == false && g.NavWindow.RootWindowForTitleBarHighlight == host_window.RootWindowDockTree && root_node.LastFocusedNodeId == node.ID {
        ...
        while parent_node != null_mut() {
            ...
            parent_node = if parent_node.HostWindow.is_null() == false {
...
// static c_void DockNodeUpdateTabBar(node: *mut ImGuiDockNode, host_window: *mut ImGuiWindow)
pub unsafe fn DockNodeUpdateTabBar(node: *mut ImGuiDockNode, host_window: *mut ImGuiWindow)
{
    ...
    PushOverrideID(node.ID);
    let mut tab_bar: *mut ImGuiTabBar =  node.TabBar;
    ...
    let title_bar_col = GetColorU32(if host_window.Collapsed { ImGuiCol_TitleBgCollapsed } else {
    if is_focused {
        ImGuiCol_TitleBgActive
    } else { ImGuiCol_TitleBg } }, 0.0);
    let rounding_flags = CalcRoundingFlagsForRectInRect(title_bar_rect, host_window.Rect(), DOCKING_SPLITTER_SIZE);
    ...
    for tab_n in tab_bar.Tabs.len() - 1 .. 0
    {
        // FIXME-DOCK: Consider only clearing the flag after the tab has been alive for a few consecutive frames, allowing late comers to not break sorting?
        tab_bar.Tabs[tab_n].Flags &= !ImGuiTabItemFlags_Unsorted;
        tabs_unsorted_start = tab_n as c_int;
        if tab_bar.Tabs[tab_n].Flags & ImGuiTabItemFlags_Unsorted == 0{
            break;
        }
    }
    ...
    if tab_bar_is_recreated && TabBarFindTabByID(tab_bar, node.SelectedTabId) != null_mut() {
        ...
        tab_bar.SelectedTabId =  tab_bar.Tabs.last().unwrap().window.TabId;
        tab_bar.NextSelectedTabId = tab_bar.Tabs.last().unwrap().window.TabId;
    }
    ...
    {
        let mut window: *mut ImGuiWindow =  node.Windows[window_n];
        if ((closed_all || closed_one == window.TabId) && window.HasCloseButton && flag_clear(window.Flags, ImGuiWindowFlags_UnsavedDocument)) {
            continue;
        }
        if (window.LastFrameActive + 1 >= g.FrameCount || !node_was_active)
        {
            let mut tab_item_flags: ImGuiTabItemFlags = 0;
    ...
    if g.HoveredId == 0 || g.HoveredId == title_bar_id || g.ActiveId == title_bar_id
    {
        ...
        ButtonBehavior(title_bar_rect.clone(), title_bar_id, null_mut(), &held, ImGuiButtonFlags_AllowItemOverlap);
        ...
        if (held)
        {
            ...
            let mut tab: *mut ImGuiTabItem = TabBarFindTabByID(tab_bar, tab_bar.SelectedTabId);
    ...
    if (focus_tab_id != 0) {
        let mut tab: *mut ImGuiTabItem = TabBarFindTabByID(tab_bar, focus_tab_id);
        if tab.is_null() == false{
            if (tab.Window) {
...
// static c_void DockNodeAddTabBar(node: *mut ImGuiDockNode)
pub fn DockNodeAddTabBar(node: *mut ImGuiDockNode)
{
    // IM_ASSERT(node.TabBar == NULL);
    node.TabBar = IM_NEW(ImGuiTabBar);
}

// static c_void DockNodeRemoveTabBar(node: *mut ImGuiDockNode)
pub fn DockNodeRemoveTabBar(node: *mut ImGuiDockNode)
{
    if (node.TabBar == null_mut()) {
        return;
    }
...
// static bool DockNodeIsDropAllowedOne(payload: *mut ImGuiWindow, host_window: *mut ImGuiWindow)
pub unsafe fn DockNodeIsDropAllowedOne(payload: *mut ImGuiWindow, host_window: *mut ImGuiWindow) -> bool
{
    ...
    let mut host_class: *mut ImGuiWindowClass = if host_window.DockNodeAsHost { &mut host_window.DockNodeAsHost.WindowClass } else { &mut host_window.WindowClass };
    let mut payload_class: *mut ImGuiWindowClass = &mut payload.WindowClass;
    ...
    {
        let mut popup_window: *mut ImGuiWindow = g.OpenPopupStack[i].Window;
...
pub unsafe fn DockNodeIsDropAllowed(host_window: *mut ImGuiWindow, root_payload: *mut ImGuiWindow) -> bool
{
    ...
    let payload_count: c_int = if root_payload.DockNodeAsHost { root_payload.DockNodeAsHost.Windows.len() } else { 1 } as c_int;
...
bool DockNodeCalcDropRectsAndTestMousePos(const ImRect& parent, dir: ImGuiDir, ImRect& out_r, outer_docking: bool, test_mouse_pos: *mut ImVec2)
{
    ...
    let mut off = ImVec2::default(); // Distance from edge or center
    if (outer_docking)
    {
...
c_void DockNodeTreeMerge(ctx: *mut ImGuiContext, parent_node: *mut ImGuiDockNode, merge_lead_child: *mut ImGuiDockNode)
{
    ...
    let g = GImGui; // ImGuiContext& g = *GImGui;
    let mut child_0: *mut ImGuiDockNode = parent_node.ChildNodes[0];
