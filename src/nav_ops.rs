...
use libc::{c_char, c_float, c_int, INT_MAX, INT_MIN};
...
use crate::window_flags::{ImGuiWindowFlags_AlwaysAutoResize, ImGuiWindowFlags_ChildMenu, ImGuiWindowFlags_ChildWindow, ImGuiWindowFlags_MenuBar, ImGuiWindowFlags_Modal, ImGuiWindowFlags_NoFocusOnAppearing, ImGuiWindowFlags_NoInputs, ImGuiWindowFlags_NoMove, ImGuiWindowFlags_NoNavInputs, ImGuiWindowFlags_NoResize, ImGuiWindowFlags_NoSavedSettings, ImGuiWindowFlags_NoTitleBar, ImGuiWindowFlags_Popup};
use crate::window_ops::{FindWindowByName, WindowRectAbsToRel, WindowRectRelToAbs};
...
pub unsafe fn NavScoreItem(result: *mut ImGuiNavItemData) -> bool {
    ...
    let mut dbx: c_float = NavScoreItemDistInterval(cand.Min.x.clone(), cand.Max.x.clone(), curr.Min.x, curr.Max.x);
...
// #if IMGUI_DEBUG_NAV_SCORING
    ...
    if IsMouseHoveringRect(&cand.Min, &cand.Max, false) {
        ...
        draw_list.AddRectFilled3(cand.Max - ImVec2::new2(4.0, 4.0), cand.Max + CalcTextSize(buf.as_ptr(), null(), false, 0.0) + ImVec2::new2(4.0, 4.0), IM_COL32(40, 0, 0, 150), 0.0, 0);
...
pub unsafe fn NavProcessItem() {
    ...
    let nav_bb: ImRect = g.LastItemData.NavRect.clone();
    ...
    if g.NavInitRequest && g.NavLayer == window.DC.NavLayerCurrent && flag_clear(item_flags, ImGuiItemFlags_Disabled) {
        ...
        let candidate_for_nav_default_focus: bool = flag_clear(item_flags, ImGuiItemFlags_NoNavDefaultFocus);
...
pub unsafe fn GetNavTweakPressedAmount(axis: ImGuiAxis) -> c_float {
    ...
    let mut amount: c_float = (GetKeyPressedAmount(key_more, repeat_delay, repeat_rate) - GetKeyPressedAmount(key_less, repeat_delay, repeat_rate)) as c_float;
...
pub unsafe fn NavUpdate() {
    ...
    if is_not_null(g.NavWindow) && flag_clear(g.NavWindow.Flags, ImGuiWindowFlags_NoNavInputs) && is_null(g.NavWindowingTarget) {
        ...
        if nav_gamepad_active {
            ...
            let tweak_factor: c_float = if IsKeyDown(ImGuiKey_NavGamepadTweakSlow) { 1f32 / 10f32 } else {
...
// #if IMGUI_DEBUG_NAV_RECTS
    if g.NavWindow {
        let mut draw_list: *mut ImDrawList = GetForegroundDrawList3(g.NavWindow);
        ...
        let mut col: u32 = if !g.NavWindow.Hidden { IM_COL32(255, 0, 255, 255) } else { IM_COL32(255, 0, 0, 255) };
        let mut p: ImVec2 = NavCalcPreferredRefPos();
        let mut buf: [c_char; 32] = [0; 32];
        // ImFormatString(buf.as_ptr(), 32, "%d", g.NavLayer);
        draw_list.AddCircleFilled(&p, 3.0f32, col, 0);
        draw_list.AddText2(null_mut(), 13.0f32, p + ImVec2::new2(8.0, -4.0), col, buf.as_ptr(), null(), 0.0, null());
    }
// #endif
}
...
pub unsafe fn NavUpdateCreateMoveRequest() {
    ...
    if g.NavMoveForwardToNextFrame && window != null_mut() {
        ...
        if is_not_null(window) && is_null(g.NavWindowingTarget) && flag_set(window.Flags, ImGuiWindowFlags_NoNavInputs) {
        ...
        g.NavScoringNoClipRect = ImRect::new4(f32::MAX, f32::MAX, f32::MIN, f32::MIN);
    }

    // Update PageUp/PageDown/Home/End scroll
    // FIXME-NAV: Consider enabling those keys even without the master ImGuiConfigFlags_NavEnableKeyboard flag?
    let mut scoring_rect_offset_y: c_float = 0f32;
    if is_not_null(window) && g.NavMoveDir == ImGuiDir_None && nav_keyboard_active {
        scoring_rect_offset_y = NavUpdatePageUpPageDown();
    }
    if scoring_rect_offset_y != 0f32 {
        g.NavScoringNoClipRect = window.InnerRect.clone();
...
// #endif
    ...
    if g.NavMoveSubmitted && g.NavInputSource == ImGuiInputSource_Gamepad && g.NavLayer == ImGuiNavLayer_Main && window != null_mut() {// && (g.NavMoveFlags & ImGuiNavMoveFlags_Forwarded)) {
        ...
        let mut inner_rect_rel: ImRect = WindowRectAbsToRel(window, &ImRect::new4(window.InnerRect.Min - ImVec2::new2(1.0, 1.0), window.InnerRect.Max + ImVec2::new2(1.0, 1.0), 0.0, 0.0));
        if (clamp_x || clamp_y) && !inner_rect_rel.Contains(window.NavRectRel[g.NavLayer]) {
            ...
            inner_rect_rel.Min.x = if clamp_x { (inner_rect_rel.Min.x + pad_x) } else { f32::MIN };
            inner_rect_rel.Max.x = if clamp_x { (inner_rect_rel.Max.x - pad_x) } else { f32::MAX };
            inner_rect_rel.Min.y = if clamp_y { (inner_rect_rel.Min.y + pad_y) } else { f32::MIN };
    ...
    if window != null_mut() {
        let nav_rect_rel: ImRect = if !window.NavRectRel[g.NavLayer].IsInverted() { window.NavRectRel[g.NavLayer] } else { ImRect::new4(0.0, 0.0, 0.0, 0.0) };
    ...
    g.NavScoringNoClipRect.Add2(&scoring_rect);
}
...
pub unsafe fn NavUpdateCreateTabbingRequest() {
    ...
    if is_null(window) || g.NavWindowingTarget != null_mut() || flag_set(window.Flags, ImGuiWindowFlags_NoNavInputs) {
        return;
    }
    ...
    let clip_dir = if g.NavTabbingDir < 0 { ImGuiDir_Up } else { ImGuiDir_Down };
...
// #endif
    ...
    let mut result = if g.NavMoveResultLocal.ID != 0 { &g.NavMoveResultLocal } else { if g.NavMoveResultOther.ID != 0 { &g.NavMoveResultOther } else { null_mut() } };
    ...
    if g.NavLayer == ImGuiNavLayer_Main {
        if g.NavMoveFlags & ImGuiNavMoveFlags_ScrollToEdgeY {
            ...
            let scroll_target: c_float = if g.NavMoveDir == ImGuiDir_Up { result.window.ScrollMax.y } else { 0f32 };
...
pub unsafe fn NavUpdateCancelRequest() {
    ...
    if g.ActiveId != 0 {
        ...
        if is_not_null(g.NavWindow) && ((g.NavWindow.Flags & ImGuiWindowFlags_Popup) || !(g.NavWindow.Flags & ImGuiWindowFlags_ChildWindow)) {
...
pub unsafe fn NavUpdatePageUpPageDown() -> c_float {
    ...
    if window.DC.NavLayersActiveMask == 0x00 && window.DC.NavHasScroll {
        ...
        let mut nav_scoring_rect_offset_y: c_float = 0f32;
        if IsKeyPressed(ImGuiKey_PageUp, true) {
...
pub unsafe fn NavEndFrame() {
    ...
    let wanted_flags = ImGuiNavMoveFlags_WrapX | ImGuiNavMoveFlags_LoopX | ImGuiNavMoveFlags_WrapY | ImGuiNavMoveFlags_LoopY;
    if is_not_null(g.NavWindow) && NavMoveRequestButNoResultYet() && flag_set(g.NavMoveFlags, wanted_flags) && flag_clear(g.NavMoveFlags, ImGuiNavMoveFlags_Forwarded) {
...
pub unsafe fn NavUpdateCreateWrappingRequest() {
    ...
    let mut clip_dir = g.NavMoveDir;
    let move_flags = g.NavMoveFlags;
    ...
    if g.NavMoveDir == ImGuiDir_Right && flag_set(move_flags,
...
pub unsafe fn FindWindowNavFocusable(i_start: c_int, i_stop: c_int, dir: c_int) -> *mut ImGuiWindow {
    let g = GImGui; // ImGuiContext& g = *GImGui;
    // for (let i: c_int = i_start; i >= 0 && i < g.WindowsFocusOrder.Size && i != i_stop; i += dir)
    while i >= 0 && i < g.WindowsFocusOrder.len() && i != i_stop {
...
pub unsafe fn NavUpdateWindowingHighlightWindow(focus_change_dir: c_int) {
    ...
    let mut window_target: *mut ImGuiWindow = FindWindowNavFocusable(i_current + focus_change_dir, INT_MIN, focus_change_dir);
    if !window_target {
...
if window_target { // Don't reset windowing target if there's a single window in the list {
        ...
        g.NavWindowingAccumDeltaPos = ImVec2::new2(0f32, 0f32);
        g.NavWindowingAccumDeltaSize = ImVec2::new2(0f32, 0f32);
    }
...
pub unsafe fn NavUpdateWindowing() {
    ...
    if start_windowing_with_gamepad || start_windowing_with_keyboard {
        let mut window: *mut ImGuiWindow = FindWindowNavFocusable(g.WindowsFocusOrder.Size - 1, -INT_MAX, -1);
        if is_not_null(window) {
            ...
            g.NavWindowingHighlightAlpha = 0f32;
            g.NavWindowingAccumDeltaPos = ImVec2::new2(0f32, 0f32);
            g.NavWindowingAccumDeltaSize = ImVec2::new2(0f32, 0f32);
    ...
    if is_not_null(g.NavWindowingTarget) && flag_clear(g.NavWindowingTarget.Flags, ImGuiWindowFlags_NoMove) {
        ...
        if nav_move_dir.x != 0f32 || nav_move_dir.y != 0f32 {
            ...
            g.NavDisableMouseHover = true;
            let accum_floored: ImVec2 = ImFloor2(&g.NavWindowingAccumDeltaPos);
    ...
    // Apply final focus
    if is_not_null(apply_focus_window) && (is_null(g.NavWindow) || apply_focus_window != g.NavWindow.RootWindow) {
        ImGuiViewport * previous_viewport = if g.NavWindow { g.NavWindow.Viewport } else { null_mut() };
        ClearActiveID();
        NavRestoreHighlightAfterMove();
    ...
    if apply_toggle_layer && is_not_null(g.NavWindow) {
        ...
        while is_not_null(new_nav_window.ParentWindow) && (new_nav_window.DC.NavLayersActiveMask & (1 << ImGuiNavLayer_Menu)) == 0 && (new_nav_window.Flags & ImGuiWindowFlags_ChildWindow) != 0 && (new_nav_window.Flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu)) == 0 {
        ...
        // Toggle layer
        let new_nav_layer = if (g.NavWindow.DC.NavLayersActiveMask & (1 << ImGuiNavLayer_Menu))? { (ImGuiNavLayer)(g.NavLayer ^ 1) } else { ImGuiNavLayer_Main };
...
pub unsafe fn GetFallbackWindowNameForWindowingList(window: *mut ImGuiWindow) -> *const c_char {
    if window.Flags & ImGuiWindowFlags_Popup {
        return str_to_const_c_char_ptr("(Popup)");
    }
    if flag_set(window.Flags, ImGuiWindowFlags_MenuBar) && libc::strcmp(window.Name, str_to_const_c_char_ptr("##MainMenuBar")) == 0 {
        return str_to_const_c_char_ptr("(Main menu bar)");
    }
    if window.DockNodeAsHost {
        return str_to_const_c_char_ptr("(Dock node)");
    }
    return str_to_const_c_char_ptr("(Untitled)");
}
...
pub unsafe fn NavUpdateWindowingOverlay() {
    ...
    SetNextWindowSizeConstraints(ImVec2::new2(viewport.Size.x * 0.20f32, viewport.Size.y * 0.200f32), ImVec2::new2(f32::MAX, f32::MAX));
