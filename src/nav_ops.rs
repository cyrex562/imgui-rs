...
use libc::{c_char, c_float, c_int, INT_MIN};
...
use crate::constants::NAV_WINDOWING_HIGHLIGHT_DELAY;
...
use crate::window_flags::{ImGuiWindowFlags_ChildMenu, ImGuiWindowFlags_ChildWindow, ImGuiWindowFlags_Modal, ImGuiWindowFlags_NoMove, ImGuiWindowFlags_NoNavInputs, ImGuiWindowFlags_Popup};
    ...
    if g.NavWindow
    {
        ...
        for layer in 0 .. 2
        {
            let r: ImRect =  WindowRectRelToAbs(g.NavWindow, &g.NavWindow.NavRectRel[layer]);
             draw_list.AddRect(&r.Min, &r.Max, IM_COL32(255, 200, 0, 255), 0.0, 0, 0.0); }  // [DEBUG]
...
pub unsafe fn NavUpdateWindowingHighlightWindow(focus_change_dir: c_int)
{
    ...
    {
        g.NavWindowingTarget = window_target;g.NavWindowingTargetAnim = window_target;
        g.NavWindowingAccumDeltaPos =  ImVec2::new2(0f32, 0f32);g.NavWindowingAccumDeltaSize = ImVec2::new2(0f32, 0f32);
    }
...
pub unsafe fn NavUpdateWindowing()
{
    ...
    if start_windowing_with_gamepad || start_windowing_with_keyboard {
        let mut window: *mut ImGuiWindow = g.NavWindow ? FindWindowNavFocusable(g.WindowsFocusOrder.Size - 1, -INT_MAX, -1))
