...
pub unsafe fn DockBuilderRemoveNodeChildNodes(root_id: ImGuiID)
{
    ...
    root_node:*mut ImGuiDockNode = if root_id { DockContextFindNodeByID(ctx, root_id) } else { null_mut() };
    if root_id && root_node == null_mut() { return ; }
