...
struct ImSpanAllocator
{
    *mut char   BasePtr;
    ...
    inline *mut c_void GetSpanPtrBegin(c_int n)             { IM_ASSERT(n >= 0 && n < CHUNKS && CurrIdx == CHUNKS); return (BasePtr + Offsets[n]); }
    inline *mut c_void GetSpanPtrEnd(c_int n)               { IM_ASSERT(n >= 0 && n < CHUNKS && CurrIdx == CHUNKS); return (BasePtr + Offsets[n] + Sizes[n]); }
    template<typename T>
