...
use crate::draw_list::DimgDrawDataBuilder;
...
pub struct DimgViewport
{
    ...
    pub last_name_hash: DimgId,
    // ImVec2              LastPos;
    pub last_pos: DimgVec2D,
    ...
    pub draw_lists: [DimgId;2],
    // ImDrawData          DrawDataP;
    pub draw_data: DimgDrawData,
    // ImDrawDataBuilder   DrawDataBuilder;
    pub draw_data_builder: DimgDrawDataBuilder,
    // ImVec2              LastPlatformPos;
    pub last_plaform_pos: DimgVec2D,
    // ImVec2              LastPlatformSize;
    pub last_platform_size: DimgVec2D,
    // ImVec2              LastRendererSize;
    pub last_renderer_size: DimgVec2D,
    // ImVec2              WorkOffsetMin;          // Work Area: Offset from pos to top-left corner of Work Area. Generally (0,0) or (0,+main_menu_bar_height). Work Area is Full Area but without menu-bars/status-bars (so WorkArea always fit inside pos/size!)
    pub work_offset_min: DimgVec2D,
    // ImVec2              WorkOffsetMax;          // Work Area: Offset from pos+size to bottom-right corner of Work Area. Generally (0,0) or (0,-status_bar_height).
    pub work_offset_max: DimgVec2D,
    // ImVec2              BuildWorkOffsetMin;     // Work Area: Offset being built during current frame. Generally >= 0.0.
    pub build_work_offset_min: DimgVec2D,
    // ImVec2              BuildWorkOffsetMax;     // Work Area: Offset being built during current frame. Generally <= 0.0.
    pub build_work_offset_max: DimgVec2D,
}
...
impl DimgViewport {
    ...
    pub fn new() -> Self {
        Self {
            ...
            window: DimgId::MAX,
            draw_lists: [0;2],
            last_platform_pos: DimgVec2D::new(f32::MAX, f32::MAX),
            last_platform_size: DimgVec2D::new(f32::MAX, y: f32::MAX),
            last_renderer_size: DmigVec2D::new(f32::MAX,f32::MAX),
...
pub struct DimgViewport
{
    ...
    pub work_pos: DimgVec2D,                // Work Area: Position of the viewport minus task bars, menus bars, status bars (>= pos)
    pub work_size: DimgVec2D,               // Work Area: size of the viewport minus task bars, menu bars, status bars (<= size)
    pub dpi_scale: f32,              // 1.0 = 96 DPI = No extra scale.
    pub parent_viewport_id: DimgId,     // (Advanced) 0: no parent. Instruct the platform backend to setup a parent/child relationship between platform windows.
...
impl DimgViewport {
    // ImVec2              get_center() const       { return ImVec2(pos.x + size.x * 0.5, pos.y + size.y * 0.5); }
    // ImVec2              get_work_center() const   { return ImVec2(work_pos.x + work_size.x * 0.5, work_pos.y + work_size.y * 0.5); }
    pub fn get_center(&self) -> DimgVec2D {
        DimgVec2D {
    ...
    pub fn get_work_center(&self) -> DimgVec2D {
        DimgVec2D {
