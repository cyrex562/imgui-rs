...
impl ImDrawListSplitter {
    ...
    pub unsafe fn Split(&mut self, draw_list: *mut ImDrawList, channels_count: size_t) {
        ...
        if old_channels_count < channels_count {
            ...
            self._Channels
                .resize_with(channels_count, ImDrawChannel::default());
        }
    ...
    pub unsafe fn Merge(&mut self, mut draw_list: *mut ImDrawList) {
        ...
        let mut last_cmd = if self._Count > 0 && draw_list.CmdBuffer.len() > 0 {
            draw_list.CmdBuffer.last_mut().unwrap()
        } else {
            None
        };
        let mut idx_offset: size_t = if last_cmd {
            last_cmd.IdxOffset + last_cmd.ElemCount
        } else {
            0
        };
        ...
        for i in 1..self._Count {
            ...
            if ch._CmdBuffer.len() > 0
                && ch._CmdBuffer.last().unwrap().ElemCount == 0
                && ch._CmdBuffer.last().unwrap().UserCallback == None
            {
            ...
            if ch._CmdBuffer.len() > 0 && last_cmd != None {
                ...
                if ImDrawCmd_HeaderCompare(last_cmd, next_cmd) == 0
                    && last_cmd.UserCallback == None
                    && next_cmd.UserCallback == None
                {
        ...
        draw_list.CmdBuffer.resize_with(
            draw_list.CmdBuffer.len() + new_cmd_buffer_count,
            ImDrawCmd::default(),
        );
        ...
        for i in 1..self._Count {
            ...
            if sz > 0 {
                libc::memcpy(cmd_write, ch._CmdBuffer, sz * mem::size_of::<ImDrawCmd>());
                cmd_write += sz;
            }
            let sz: size_t = ch._IdxBuffer.len();
            if sz > 0 {
                libc::memcpy(idx_write, ch._IdxBuffer, sz * sizeof);
                idx_write += sz;
            }
        }
        ...
        if draw_list.CmdBuffer.len() == 0
            || draw_list.CmdBuffer.last().unwrap().UserCallback != None
        {
    ...
    pub unsafe fn SetCurrentChannel(&mut self, draw_list: &mut ImDrawList, channel_idx: c_int) {
        ...
        libc::memcpy(
            &mut self._Channels[self._Current]._CmdBuffer,
            &draw_list.CmdBuffer,
            sizeof(draw_list.CmdBuffer.as_ptr()),
        );
        libc::memcpy(
            &mut self._Channels[self._Current]._IdxBuffer,
            &draw_list.IdxBuffer,
            sizeof(draw_list.IdxBuffer.as_ptr()),
        );
        _Current = idx;
        libc::memcpy(
            &mut draw_list.CmdBuffer,
            &self._Channels[idx]._CmdBuffer,
            sizeof(draw_list.CmdBuffer.as_ptr()),
        );
        libc::memcpy(
            &mut draw_list.IdxBuffer,
            &self._Channels[idx]._IdxBuffer,
            sizeof(draw_list.IdxBuffer.as_ptr()),
        );
        draw_list._IdxWritePtr = draw_list.IdxBuffer.as_ptr() + draw_list.IdxBuffer.len();

        // If current command is used with different settings we need to add a new command
        let mut curr_cmd: *mut ImDrawCmd = if (draw_list.CmdBuffer.len() == 0) {
            None
        } else {
            &mut draw_list.CmdBuffer[draw_list.CmdBuffer.len() - 1]
        };
