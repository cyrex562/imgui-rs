...
use crate::imgui_color::{
    color_convert_float4_to_u32, ImGuiColorMod, COLOR_32_A_MASK, IM_COL32_A_SHIFT,
};
...
impl Style {
    ...
    pub fn scale_all_sizes(&mut self, scale_factor: f32) {
        ...
        self.TabMinWidthForCloseButton = if Self.TabMinWidthForCloseButton != f32::MAX {
...
pub fn GetColorU32_3(col: u32) -> u32 {
    ...
    let mut a = (col & COLOR_32_A_MASK) >> IM_COL32_A_SHIFT;
    a = (a * style.alpha); // We don't need to clamp 0..255 because style.Alpha is in 0..1 range.
    (col & !COLOR_32_A_MASK) | (a << IM_COL32_A_SHIFT)
}
...
pub fn StyleColorsDark(dst: *mut Style) {
    ...
    colors[ImGuiColor::Tab] = ImLerpF32(
        colors[ImGuiColor::Header],
        colors[ImGuiColor::TitleBgActive],
        0.80,
    );
    colors[ImGuiColor::TabHovered] = colors[ImGuiColor::HeaderHovered];
    colors[ImGuiColor::TabActive] = ImLerpF32(
        colors[ImGuiColor::HeaderActive],
        colors[ImGuiColor::TitleBgActive],
        0.60,
    );
    colors[ImGuiColor::TabUnfocused] =
        ImLerpF32(colors[ImGuiColor::Tab], colors[ImGuiColor::TitleBg], 0.80);
    colors[ImGuiColor::TabUnfocusedActive] = ImLerpF32(
        colors[ImGuiColor::TabActive],
        colors[ImGuiColor::TitleBg],
        0.40,
    );
    colors[ImGuiColor::DockingPreview] =
        colors[ImGuiColor::HeaderActive] * Vector4D::new(1.0, 1.0, 1.0, 0.7);
...
pub fn StyleColorsClassic(dst: *mut Style) {
    ...
    colors[ImGuiColor::DockingPreview] =
        colors[ImGuiColor::Header] * Vector4D::new(1.0, 1.0, 1.0, 0.7);
    colors[ImGuiColor::DockingEmptyBg] = Vector4D::new(0.20, 0.20, 0.20, 1.00);
