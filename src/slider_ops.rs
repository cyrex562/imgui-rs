...
use crate::item_ops::{CalcItemWidth, ItemAdd, ItemHoverable, ItemSize, MarkItemEdited, PopItemWidth, PushMultiItemsWidths};
...
pub unsafe fn SliderBehaviorT(bb: &ImRect, id: ImGuiID, data_type: ImGuiDataType, v: &mut c_float, v_min: c_float, v_max: c_float, format: &str, flags: ImGuiSliderFlags, out_grab_bb: &mut ImRect) -> bool
{
    ...
    else
    {
        ...
        let grab_pos: c_float =  ImLerp(slider_usable_pos_min, slider_usable_pos_max, grab_t);
        if axis == ImGuiAxis_X {
            *out_grab_bb = ImRect::from_floats(grab_pos - grab_sz * 0.5,
                                             bb.Min.y + grab_padding,
                                             grab_pos + grab_sz * 0.5,
                                             bb.Max.y - grab_padding);
        }
        else {
            *out_grab_bb = ImRect::from_floats(bb.Min.x + grab_padding, grab_pos - grab_sz * 0.5, bb.Max.x - grab_padding, grab_pos + grab_sz * 0.5);
        }
    }
...
pub unsafe fn SliderBehavior(bb: &ImRect,
                             id: ImGuiID,
                             ...
                             out_grab_bb: &mut ImRect) -> bool
{
    ...
    if flag_set(g.LastItemData.InFlags, ImGuiItemFlags_ReadOnly)
        || flag_set(flags, ImGuiSliderFlags_ReadOnly) { return  false; }

    SliderBehaviorT(bb, id, ImGuiDataType_S32, p_v, p_min,  p_max,  format, flags, out_grab_bb)
...
pub unsafe fn SliderScalar(label: &str,
                           data_type: ImGuiDataType,
                           p_data: &mut c_float,
                           p_min: c_float,
                           p_max: c_float,
                           ...
                           flags: ImGuiSliderFlags) -> bool
{
    ...
    let mut frame_bb: ImRect = ImRect::new(window.DC.CursorPos,
                                           window.DC.CursorPos + ImVec2::new(w, label_size.y + style.FramePadding.y * 2.0));
    let mut total_bb: ImRect = ImRect::new(frame_bb.Min,
                                           frame_bb.Max + ImVec2::new(if label_size.x > 0.0 {
                                               style.ItemInnerSpacing.x + label_size.x} else {
                                               0.0}, 0.0));
    ...
    if temp_input_is_active
    {
        ...
        return input_num_ops::TempInputScalar(&mut frame_bb, id, label, data_type, p_data, format, if is_clamp_input { p_min} else {null_mut()}, if is_clamp_input { p_max} else {null_mut()});
    }

    // Draw frame
    frame_col: u32 = GetColorU32(if g.ActiveId == id { ImGuiCol_FrameBgActive }
                                 else { if hovered { ImGuiCol_FrameBgHovered }
    ...
    let value_changed: bool = SliderBehavior(&frame_bb, id, data_type, p_data, p_min, p_max, format, flags, &mut grab_bb);
    if value_changed {
        MarkItemEdited(id); }

    // Render grab
    if grab_bb.Max.x > grab_bb.Min.x {
        window.DrawList.AddRectFilled(&grab_bb.Min, &grab_bb.Max, GetColorU32(if g.ActiveId == id {
            ImGuiCol_SliderGrabActive
        } else {
            ImGuiCol_SliderGrab
        },
                                                                               0.0), style.GrabRounding, 0);
    }
    ...
    if label_size.x > 0.0 {
        RenderText(ImVec2::new(frame_bb.Max.x + style.ItemInnerSpacing.x,
...
pub unsafe fn SliderScalarN(label: &str,
                            data_type: ImGuiDataType,
                            v: &mut [c_float],
                            components: usize,
                            v_min: &mut [c_float],
...
pub unsafe fn SliderFloat(label: &str,
                          v: &mut c_float,
                          v_min: c_float,
                          v_max: c_float,
                          format: &mut String,
...
pub unsafe fn SliderFloat2(label: &str,
                           v: &mut [c_float;2],
                           v_min: &mut [c_float;2],
                           v_max: &mut [c_float;2],
                           format: &str, flags: ImGuiSliderFlags) -> bool
{
    return SliderScalarN(label, ImGuiDataType_Float, v, 2, v_min, v_max, format, flags);
}

pub unsafe fn SliderFloat3(label: &str,
                           v: &mut [c_float;3],
                           v_min: &mut [c_float;3],
                           v_max: &mut [c_float;3],
                           format: &str,
...
pub unsafe fn SliderFloat4(label: &str,
                           v: &mut [c_float;4],
                           v_min: &mut [c_float;4],
                           v_max: &mut [c_float;4],
                           format: &str,
...
pub unsafe fn SliderAngle(label: &str,
                          v_rad: &mut c_float,
                          ...
                          flags: ImGuiSliderFlags) -> bool
{
    ...
    let mut value_changed: bool =  SliderFloat(label,
                                               &mut v_deg,
                                               v_degrees_min,
                                               v_degrees_max,
                                               format,
...
pub unsafe fn SliderInt(label: &str,
                        v: &mut c_int,
                        v_min: &mut c_int,
                        v_max: &mut c_int,
                        format: &mut String,
                        flags: ImGuiSliderFlags) -> bool
{
    ...
    return SliderScalar(label, ImGuiDataType_S32, &mut v_float, v_min_float, v_max_float, format, flags);
}

pub unsafe fn SliderInt2(label: &str, v: &mut [c_int; 2], v_min: &mut [c_int; 2], v_max: &mut [c_int; 2], format: &str, flags: ImGuiSliderFlags) -> bool {
    ...
    return SliderScalarN(label, ImGuiDataType_S32, &mut v_float, 2, &mut v_min_float, &mut v_max_float, format, flags);
}

pub unsafe fn SliderInt3(label: &str, v: [c_int;3], v_min: c_int, v_max: c_int, format: &str, flags: ImGuiSliderFlags) -> bool
{
    let mut v_float: [c_float;3] = [v[0] as c_float, v[1] as c_float, v[2] as c_float];
    let mut v_min_float: [c_float;3] = [v_min[0] as c_float, v_min[1] as c_float, v_min[2] as c_float];
    let mut v_max_float: [c_float;3] = [v_max[0] as c_float, v_max[1] as c_float, v_max[2] as c_float];
    return SliderScalarN(label, ImGuiDataType_S32, &mut v_float, 3, &mut v_min_float, &mut v_max_float, format, flags);
}

pub unsafe fn SliderInt4(label: &str, v: [c_int;4], v_min: c_int, v_max: c_int, format: &str, flags: ImGuiSliderFlags) -> bool
{
    let mut v_float: [c_float;4] = [v[0] as c_float, v[1] as c_float, v[2] as c_float, v[3] as c_float];
    let mut v_min_float: [c_float;4] = [v_min[0] as c_float, v_min[1] as c_float, v_min[2] as c_float, v_min[3] as c_float];
    let mut v_max_float: [c_float;4] = [v_max[0] as c_float, v_max[1] as c_float, v_max[2] as c_float, v_max[3] as c_float];
    return SliderScalarN(label, ImGuiDataType_S32, &mut v_float, 4, &mut v_min_float, &mut v_max_float, format, flags);
}

pub unsafe fn VSliderScalar(label: &str,
                            size: &ImVec2,
                            data_type: ImGuiDataType,
                            p_data: &mut c_float,
                            p_min: c_float,
                            ...
                            flags: ImGuiSliderFlags) -> bool
{
    ...
    if !ItemAdd(&mut frame_bb, id, null(), 0) { return  false; }
    ...
    // Draw frame
    frame_col: u32 = GetColorU32(if g.ActiveId == id { ImGuiCol_FrameBgActive} else {if hovered { ImGuiCol_FrameBgHovered}else{ ImGuiCol_FrameBg}}, 0.0);
    RenderNavHighlight(&frame_bb, id, 0);
    RenderFrame(frame_bb.Min, frame_bb.Max, frame_col, true, g.Style.FrameRounding);

    // Slider behavior
    let mut grab_bb: ImRect = ImRect::default();
    let value_changed: bool = SliderBehavior(&frame_bb, id, data_type, p_data, p_min, p_max, format, flags | ImGuiSliderFlags_Vertical, &mut grab_bb);
    if value_changed {
        MarkItemEdited(id); }

    // Render grab
    if grab_bb.Max.y > grab_bb.Min.y {
        window.DrawList.AddRectFilled(&grab_bb.Min, &grab_bb.Max, GetColorU32(if g.ActiveId == id { ImGuiCol_SliderGrabActive } else { ImGuiCol_SliderGrab }, 0.0), style.GrabRounding, 0);
    }
...
pub unsafe fn VSliderFloat(label: &str, size: &ImVec2, v: &mut c_float, v_min: c_float, v_max: c_float, format: &mut String, flags: ImGuiSliderFlags) -> bool
{
    return VSliderScalar(label, size, ImGuiDataType_Float, v, v_min, v_max, format, flags);
}

pub unsafe fn VSliderInt(label: &str, size: &ImVec2, v: &mut c_int, v_min: &mut c_int, v_max: &mut c_int, format: &mut String, flags: ImGuiSliderFlags) -> bool
{
    let mut v_float = v as c_float;
    let mut v_min_float = v as c_float;
    let mut v_max_float = v as c_float;
    return VSliderScalar(label, size, ImGuiDataType_S32, &mut v_float, v_min_float, v_max_float, format, flags);
}

// Convert a value v in the output space of a slider into a parametric position on the slider itself (the logical opposite of ScaleValueFromRatioT)
pub fn ScaleRatioFromValueT(data_type: ImGuiDataType, v: &mut c_float, mut v_min: c_float, mut v_max: c_float, is_logarithmic: bool, logarithmic_zero_epsilon: c_float, zero_deadzone_halfsize: c_float) -> c_float {
    if v_min == v_max {
        return 0.0;
    }
    // IM_UNUSED(data_type);
