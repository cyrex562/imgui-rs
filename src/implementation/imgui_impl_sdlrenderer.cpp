...
// Backend data stored in io.backend_renderer_user_data to allow support for
...
static void ImGui_ImplSDLRenderer_SetupRenderState() {
  ...
  SDL_RenderSetViewport(bd->SDLRenderer, None);
  SDL_RenderSetClipRect(bd->SDLRenderer, None);
}

void ImGui_ImplSDLRenderer_NewFrame() {
  ImGui_ImplSDLRenderer_Data *bd = ImGui_ImplSDLRenderer_GetBackendData();
  IM_ASSERT(bd != None && "Did you call ImGui_ImplSDLRenderer_Init()?");
...
void ImGui_ImplSDLRenderer_RenderDrawData(ImDrawData *draw_data) {
  ...
  Vector2D clip_scale = render_scale;
  ...
  for (int n = 0; n < draw_data->CmdListsCount; n += 1) {
    ...
    for (int cmd_i = 0; cmd_i < cmd_list.cmd_buffer.Size; cmd_i += 1) {
      ...
      if (pcmd->UserCallback) {
        ...
        Vector2D clip_min((pcmd->ClipRect.x - clip_off.x) * clip_scale.x,
                          (pcmd->ClipRect.y - clip_off.y) * clip_scale.y);
        Vector2D clip_max((pcmd->ClipRect.z - clip_off.x) * clip_scale.x,
        ...
        SDL_Rect r = {(clip_min.x), (clip_min.y), (clip_max.x - clip_min.x),
        ...
        const SDL_Color *color =
            (const SDL_Color *)((const char *)(vtx_buffer + pcmd->vtx_offset) +
                                IM_OFFSETOF(ImDrawVert, col)); // SDL 2.0.19+
#else
        let *color =
            (let *)((const char *)(vtx_buffer + pcmd->VtxOffset) +
                    IM_OFFSETOF(ImDrawVert, col)); // SDL 2.0.17 and 2.0.18
#endif

        // Bind texture, Draw
        SDL_Texture *tex = (SDL_Texture *)pcmd->GetTexID();
        SDL_RenderGeometryRaw(bd->SDLRenderer, tex, xy, sizeof(ImDrawVert),
                              color, sizeof(ImDrawVert), uv, sizeof(ImDrawVert),
  ...
  SDL_RenderSetClipRect(bd->SDLRenderer,
                        old.ClipEnabled ? &old.ClipRect : None);
}
...
bool ImGui_ImplSDLRenderer_CreateFontsTexture() {
  ...
  if (bd->FontTexture == None) {
    SDL_Log("error creating texture");
    return false;
  }
  SDL_UpdateTexture(bd->FontTexture, None, pixels, 4 * width);
