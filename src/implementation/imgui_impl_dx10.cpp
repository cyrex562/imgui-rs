...
void    ImGui_ImplDX10_InvalidateDeviceObjects()
{
    ...
    if (bd->pFontTextureView)       { bd->pFontTextureView->Release(); bd->pFontTextureView = NULL; ImGui::GetIO().Fonts->SetTexID(NULL); } // We copied bd->pFontTextureView to io.fonts->tex_id so let's clear that as well.
    if (bd->pIB)                    { bd->pIB->Release(); bd->pIB = NULL; }
    if (bd->pVB)                    { bd->pVB->Release(); bd->pVB = NULL; }
...
static void ImGui_ImplDX10_CreateWindow(ImGuiViewport* viewport)
{
    ...
    // Some backends will leave platform_handle_raw NULL, in which case we assume platform_handle will contain the HWND.
...
{
    // The main viewport (owned by the application) will always have renderer_user_data == NULL here since we didn't create the data for it.
