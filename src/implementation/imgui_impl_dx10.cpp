...
//  2021-05-19: DirectX10: Replaced direct access to ImDrawCmd::texture_id with a call to ImDrawCmd::get_tex_id(). (will become a requirement)
...
//  2018-06-08: DirectX10: Use draw_data->display_pos and draw_data->display_size to setup projection matrix and clipping rectangle.
//  2018-04-09: Misc: Fixed erroneous call to io.fonts->clear_input_data() + clear_tex_data() that was left in DX10 example but removed in 1.47 (Nov 2015) on other backends.
...
{
    return ImGui::GetCurrentContext() ? (ImGui_ImplDX10_Data*)ImGui::GetIO().Backendrenderer_user_data : None;
}
...
void ImGui_ImplDX10_RenderDrawData(ImDrawData* draw_data)
{
    ...
    {
        if (bd->pVB) { bd->pVB->Release(); bd->pVB = None; }
        ...
        desc.MiscFlags = 0;
        if (ctx->CreateBuffer(&desc, None, &bd->pVB) < 0)
            return;
    }

    if (!bd->pIB || bd->IndexBufferSize < draw_data->TotalIdxCount)
    {
        if (bd->pIB) { bd->pIB->Release(); bd->pIB = None; }
        ...
        desc.CPUAccessFlags = D3D10_CPU_ACCESS_WRITE;
        if (ctx->CreateBuffer(&desc, None, &bd->pIB) < 0)
            return;
    }
    ...
    // Our visible imgui space lies from draw_data->display_pos (top left) to draw_data->display_pos+data_data->display_size (bottom right). display_pos is (0,0) for single viewport apps.
    {
...
static void ImGui_ImplDX10_CreateFontsTexture()
{
    ...
    // Upload texture to graphics system
    {
        ...
        ID3D10Texture2D* pTexture = None;
        D3D10_SUBRESOURCE_DATA subResource;
        subResource.pSysMem = pixels;
        subResource.SysMemPitch = desc.Width * 4;
        subResource.SysMemSlicePitch = 0;
        bd->pd3dDevice->CreateTexture2D(&desc, &subResource, &pTexture);
        IM_ASSERT(pTexture != None);
...
bool    ImGui_ImplDX10_CreateDeviceObjects()
{
    ...
    // Create the vertex shader
    {
        ...
        if (FAILED(D3DCompile(vertexShader, strlen(vertexShader), None, None, None, "main", "vs_4_0", 0, 0, &vertexShaderBlob, None)))
        ...
        // Create the constant buffer
        {
            ...
            desc.CPUAccessFlags = D3D10_CPU_ACCESS_WRITE;
            desc.MiscFlags = 0;
            bd->pd3dDevice->CreateBuffer(&desc, None, &bd->pVertexConstantBuffer);
        }
    }
    ...
    // Create the pixel shader
    {
        ...
        if (FAILED(D3DCompile(pixelShader, strlen(pixelShader), None, None, None, "main", "ps_4_0", 0, 0, &pixelShaderBlob, None)))
            return false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
...
void    ImGui_ImplDX10_InvalidateDeviceObjects()
{
    ...
    if (bd->pFontSampler)           { bd->pFontSampler->Release(); bd->pFontSampler = None; }
    if (bd->pFontTextureView)       { bd->pFontTextureView->Release(); bd->pFontTextureView = None; ImGui::GetIO().Fonts->SetTexID(None); } // We copied bd->pFontTextureView to io.fonts->tex_id so let's clear that as well.
    if (bd->pIB)                    { bd->pIB->Release(); bd->pIB = None; }
    if (bd->pVB)                    { bd->pVB->Release(); bd->pVB = None; }
    if (bd->pBlendState)            { bd->pBlendState->Release(); bd->pBlendState = None; }
    if (bd->pDepthStencilState)     { bd->pDepthStencilState->Release(); bd->pDepthStencilState = None; }
    if (bd->pRasterizerState)       { bd->pRasterizerState->Release(); bd->pRasterizerState = None; }
    if (bd->pPixelShader)           { bd->pPixelShader->Release(); bd->pPixelShader = None; }
    if (bd->pVertexConstantBuffer)  { bd->pVertexConstantBuffer->Release(); bd->pVertexConstantBuffer = None; }
    if (bd->pInputLayout)           { bd->pInputLayout->Release(); bd->pInputLayout = None; }
    if (bd->pVertexShader)          { bd->pVertexShader->Release(); bd->pVertexShader = None; }
}
...
bool    ImGui_ImplDX10_Init(ID3D10Device* device)
{
    ...
    IDXGIDevice* pDXGIDevice = None;
    IDXGIAdapter* pDXGIAdapter = None;
    IDXGIFactory* pFactory = None;
...
{
    ImGui_ImplDX10_Data* bd = ImGui_ImplDX10_GetBackendData();
    IM_ASSERT(bd != None && "Did you call ImGui_ImplDX10_Init()?");
...
struct ImGui_ImplDX10_ViewportData
{
    ...
    ImGui_ImplDX10_ViewportData()   { SwapChain = None; RTView = None; }
    ~ImGui_ImplDX10_ViewportData()  { IM_ASSERT(SwapChain == None && RTView == None); }
};
...
static void ImGui_ImplDX10_CreateWindow(ImGuiViewport* viewport)
{
    ...
    IM_ASSERT(vd->SwapChain == None && vd->RTView == None);
    ...
    if (vd->SwapChain)
    {
        ...
        bd->pd3dDevice->CreateRenderTargetView(pBackBuffer, None, &vd->RTView);
        pBackBuffer->Release();
    }
}
...
{
    // The main viewport (owned by the application) will always have renderer_user_data == None here since we didn't create the data for it.
    if (ImGui_ImplDX10_ViewportData* vd = (ImGui_ImplDX10_ViewportData*)viewport->renderer_user_data)
    {
        if (vd->SwapChain)
...
static void ImGui_ImplDX10_SetWindowSize(ImGuiViewport* viewport, Vector2D size)
{
    ImGui_ImplDX10_Data* bd = ImGui_ImplDX10_GetBackendData();
    ImGui_ImplDX10_ViewportData* vd = (ImGui_ImplDX10_ViewportData*)viewport->renderer_user_data;
    if (vd->RTView)
    {
        vd->RTView->Release();
        vd->RTView = None;
    ...
    if (vd->SwapChain)
    {
        ID3D10Texture2D* pBackBuffer = None;
        vd->SwapChain->ResizeBuffers(0, (UINT)size.x, (UINT)size.y, DXGI_FORMAT_UNKNOWN, 0);
        vd->SwapChain->GetBuffer(0, IID_PPV_ARGS(&pBackBuffer));
        if (pBackBuffer == None) { fprintf(stderr, "ImGui_ImplDX10_SetWindowSize() failed creating buffers.\n"); return; }
        bd->pd3dDevice->CreateRenderTargetView(pBackBuffer, None, &vd->RTView);
        pBackBuffer->Release();
    }
}
...
static void ImGui_ImplDX10_RenderViewport(ImGuiViewport* viewport, void*)
{
    ...
    bd->pd3dDevice->OMSetRenderTargets(1, &vd->RTView, None);
...
void ImGui_ImplDX10_InitPlatformInterface()
{
    ...
    platform_io.renderer_destroy_window = ImGui_ImplDX10_DestroyWindow;
    platform_io.Renderer_SetWindowSize = ImGui_ImplDX10_SetWindowSize;
    platform_io.renderer_render_window = ImGui_ImplDX10_RenderViewport;
    platform_io.renderer_swap_buffers = ImGui_ImplDX10_SwapBuffers;
}
