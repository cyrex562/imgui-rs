...
//  Gamepad support. Enabled with 'io.config_flags |=
//  ImGuiConfigFlags_NavEnableGamepad'. [x] Platform: Mouse cursor shape and
//  visibility. Disable with 'io.config_flags |=
...
//  ImGuiPlatformIO interface. 2022-03-22: Inputs: Fix mouse position issues
//  when dragging outside of boundaries. SDL_CaptureMouse() erroneously still
//  gives out LEAVE events when hovering OS decorations. 2022-03-22: Inputs:
//  Added support for extra mouse buttons (SDL_BUTTON_X1/SDL_BUTTON_X2).
//  2022-02-04: Added SDL_Renderer* parameter to
...
//  io.AddKeyModsEvent() (added two weeks ago) with io.add_key_event() using
//  ImGuiKey_ModXXX flags. Sorry for the confusion. 2021-01-20: Inputs: calling
//  new io.add_key_analog_event() for gamepad support, instead of writing
//  directly to io.nav_inputs[]. 2022-01-17: Inputs: calling new
...
//  obsoleted. 2022-01-10: Inputs: calling new io.add_key_event(),
//  io.AddKeyModsEvent() + io.set_key_event_native_data() API (1.87+). Support
//  for full ImGuiKey range. 2021-08-17: Calling io.add_focus_event() on
//  SDL_WINDOWEVENT_FOCUS_GAINED/SDL_WINDOWEVENT_FOCUS_LOST. 2021-07-29: Inputs:
//  mouse_pos is correctly reported when the host platform window is hovered but
...
//  Misc: Preserve display_framebuffer_scale when main window is minimized.
//  2018-12-21: Inputs: Workaround for Android/iOS which don't seem to handle
//  focus related calls. 2018-11-30: Misc: Setting up io.backend_platform_name
//  so it can be displayed in the About window. 2018-11-14: Changed the
//  signature of ImGui_ImplSDL2_ProcessEvent() to take a 'const SDL_Event*'.
...
//  SDL_GetClipboardText). 2018-03-20: Misc: Setup io.backend_flags
...
//  ImGuiKey_Insert. 2017-08-25: Inputs: mouse_pos set to -FLT_MAX,-FLT_MAX when
...
static ImGui_ImplSDL2_Data *ImGui_ImplSDL2_GetBackendData() {
  return ImGui::GetCurrentContext()
             ? (ImGui_ImplSDL2_Data *)ImGui::GetIO().Backendplatform_user_data
             : None;
}
...
// You can read the io.want_capture_mouse, io.want_capture_keyboard flags to
// tell if dear imgui wants to use your inputs.
// - When io.want_capture_mouse is true, do not dispatch mouse input data to
// your main application, or clear/overwrite your copy of the mouse data.
// - When io.want_capture_keyboard is true, do not dispatch keyboard input data
...
bool ImGui_ImplSDL2_ProcessEvent(const SDL_Event *event) {
  ...
  case SDL_MOUSEWHEEL: {
    float wheel_x = (event->wheel.x > 0)   ? 1.0
                    : (event->wheel.x < 0) ? -1.0
                                           : 0.0;
    float wheel_y = (event->wheel.y > 0)   ? 1.0
                    : (event->wheel.y < 0) ? -1.0
                                           : 0.0;
  ...
  case SDL_WINDOWEVENT: {
    ...
    if (window_event == SDL_WINDOWEVENT_CLOSE ||
      ...
      if (ImGuiViewport *viewport = ImGui::FindViewportByplatform_handle(
...
static bool ImGui_ImplSDL2_Init(SDL_Window *window, SDL_Renderer *renderer,
  ...
  main_viewport->platform_handle = (void *)window;
  SDL_SysWMinfo info;
  SDL_VERSION(&info.version);
  if (SDL_GetWindowWMInfo(window, &info)) {
#ifdef _WIN32
    main_viewport->platform_handleRaw = (void *)info.info.win.window;
#elif defined(__APPLE__)
    main_viewport->platform_handleRaw = (void *)info.info.cocoa.window;
#endif
  }
...
bool ImGui_ImplSDL2_InitForOpenGL(SDL_Window *window, void *sdl_gl_context) {
  return ImGui_ImplSDL2_Init(window, None, sdl_gl_context);
}
...
#endif
  if (!ImGui_ImplSDL2_Init(window, None, None))
    return false;
  ImGui_ImplSDL2_Data *bd = ImGui_ImplSDL2_GetBackendData();
  bd->UseVulkan = true;
  return true;
...
bool ImGui_ImplSDL2_InitForD3D(SDL_Window *window) {
  ...
  IM_ASSERT(0 && "Unsupported");
#endif
  return ImGui_ImplSDL2_Init(window, None, None);
}
...
bool ImGui_ImplSDL2_InitForSDLRenderer(SDL_Window *window,
                                       SDL_Renderer *renderer) {
  return ImGui_ImplSDL2_Init(window, renderer, None);
}
...
static void ImGui_ImplSDL2_UpdateMouseData() {
  ...
  if (is_app_focused) {
...
#if SDL_HAS_CAPTURE_AND_GLOBAL_MOUSE
      if (io.config_flags & ImGuiConfigFlags_ViewportsEnable)
        SDL_WarpMouseGlobal(io.mouse_pos.x, io.mouse_pos.y);
      else
#endif
    ...
    if (bd->MouseCanUseGlobalState && bd->MouseButtonsDown == 0) {
      ...
      // coordinates (io.mouse_pos is (0,0) when the mouse is on the upper-left
  ...
  // backend, Dear imGui will ignore this field and infer the information using
  ...
  if (io.BackendFlags & ImGuiBackendFlags_HasMouseHoveredViewport) {
    ...
    if (SDL_Window *sdl_mouse_window = SDL_GetWindowFromID(bd->MouseWindowID))
      if (ImGuiViewport *mouse_viewport =
              ImGui::FindViewportByplatform_handle((void *)sdl_mouse_window))
...
#define MAP_ANALOG(KEY_NO, AXIS_NO, V0, V1)                                    \
  {                                                                            \
    float vn =                                                                 \
        ...
        (float)(V1 - V0);                                                      \
    vn = IM_SATURATE(vn);                                                      \
    io.AddKeyAnalogEvent(KEY_NO, vn > 0.1, vn);                                \
  }
  let thumb_dead_zone = 8000; // SDL_gamecontroller.h suggests using this value.
  ...
  MAP_ANALOG(ImGuiKey_GamepadL2, SDL_CONTROLLER_AXIS_TRIGGERLEFT, 0.0, 32767);
  MAP_ANALOG(ImGuiKey_GamepadR2, SDL_CONTROLLER_AXIS_TRIGGERRIGHT, 0.0, 32767);
...
static void ImGui_ImplSDL2_UpdateMonitors() {
  ...
  for (int n = 0; n < display_count; n += 1) {
    ...
    monitor.MainPos = monitor.WorkPos = DimgVec2D::new ((float)r.x, (float)r.y);
    monitor.MainSize = monitor.WorkSize =
        DimgVec2D::new ((float)r.w, (float)r.h);
#if SDL_HAS_USABLE_DISPLAY_BOUNDS
    SDL_GetDisplayUsableBounds(n, &r);
    monitor.WorkPos = DimgVec2D::new ((float)r.x, (float)r.y);
    monitor.WorkSize = DimgVec2D::new ((float)r.w, (float)r.h);
#endif
#if SDL_HAS_PER_MONITOR_DPI
    // FIXME-VIEWPORT: On MacOS SDL reports actual monitor DPI scale, ignoring
    // OS configuration. We may want to set
    //  dpi_scale to cocoa_window.backingScaleFactor here.
    float dpi = 0.0;
    if (!SDL_GetDisplayDPI(n, &dpi, None, None))
      monitor.dpi_scale = dpi / 96.0;
#endif
...
void ImGui_ImplSDL2_NewFrame() {
  ImGui_ImplSDL2_Data *bd = ImGui_ImplSDL2_GetBackendData();
  IM_ASSERT(bd != None && "Did you call ImGui_ImplSDL2_Init()?");
  ImGuiIO &io = ImGui::GetIO();
  ...
  if (w > 0 && h > 0)
    io.DisplayFramebufferScale =
        DimgVec2D::new ((float)display_w / w, (float)display_h / h);
...
struct ImGui_ImplSDL2_ViewportData {
  ...
  ~ImGui_ImplSDL2_ViewportData() {
    IM_ASSERT(Window == None && GLContext == None);
  }
};
...
static void ImGui_ImplSDL2_CreateWindow(ImGuiViewport *viewport) {
  ...
  ImGui_ImplSDL2_ViewportData *main_viewport_data =
      (ImGui_ImplSDL2_ViewportData *)main_viewport->platform_user_data;

  // Share GL resources with main context
  bool use_opengl = (main_viewport_data->GLContext != None);
  SDL_GLContext backup_context = None;
  if (use_opengl) {
...
#endif
  vd->Window =
      SDL_CreateWindow("No Title Yet", viewport->Pos.x, viewport->Pos.y,
                       viewport->Size.x, viewport->Size.y, sdl_flags);
  vd->WindowOwned = true;
  if (use_opengl) {
...
#if defined(_WIN32)
    viewport->platform_handleRaw = info.info.win.window;
#elif defined(__APPLE__)
    viewport->platform_handleRaw = (void *)info.info.cocoa.window;
#endif
  }
}
...
static void ImGui_ImplSDL2_DestroyWindow(ImGuiViewport *viewport) {
  if (ImGui_ImplSDL2_ViewportData *vd =
          (ImGui_ImplSDL2_ViewportData *)viewport->platform_user_data) {
...
static void ImGui_ImplSDL2_SetWindowSize(ImGuiViewport *viewport,
                                         Vector2D size) {
  ImGui_ImplSDL2_ViewportData *vd =
      (ImGui_ImplSDL2_ViewportData *)viewport->platform_user_data;
...
static void ImGui_ImplSDL2_InitPlatformInterface(SDL_Window *window,
  ...
  platform_io.platform_destroy_window = ImGui_ImplSDL2_DestroyWindow;
  platform_io.Platform_ShowWindow = ImGui_ImplSDL2_ShowWindow;
  platform_io.Platform_SetWindowPos = ImGui_ImplSDL2_SetWindowPos;
  platform_io.platform_get_window_pos = ImGui_ImplSDL2_GetWindowPos;
  platform_io.Platform_SetWindowSize = ImGui_ImplSDL2_SetWindowSize;
  platform_io.platform_get_window_size = ImGui_ImplSDL2_GetWindowSize;
  platform_io.platform_set_window_focus = ImGui_ImplSDL2_SetWindowFocus;
  platform_io.platform_get_window_focus = ImGui_ImplSDL2_GetWindowFocus;
  platform_io.platform_get_window_minimized = ImGui_ImplSDL2_GetWindowMinimized;
  platform_io.Platform_SetWindowTitle = ImGui_ImplSDL2_SetWindowTitle;
  platform_io.platform_render_window = ImGui_ImplSDL2_RenderWindow;
  platform_io.platform_swap_buffers = ImGui_ImplSDL2_SwapBuffers;
#if SDL_HAS_WINDOW_ALPHA
  platform_io.Platform_SetWindowAlpha = ImGui_ImplSDL2_SetWindowAlpha;
#endif
#if SDL_HAS_VULKAN
  platform_io.platform_create_vk_surface = ImGui_ImplSDL2_CreateVkSurface;
#endif
  ...
  main_viewport->platform_handle = vd->Window;
}
