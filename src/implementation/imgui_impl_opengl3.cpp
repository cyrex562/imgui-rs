...
//  2021-05-19: OpenGL: Replaced direct access to ImDrawCmd::texture_id with a call to ImDrawCmd::get_tex_id(). (will become a requirement)
...
//  2019-02-11: OpenGL: Projecting clipping rectangles correctly using draw_data->framebuffer_scale to allow multi-viewports for retina display.
...
//  2018-06-08: OpenGL: Use draw_data->display_pos and draw_data->display_size to setup projection matrix and clipping rectangle.
//  2018-05-25: OpenGL: Removed unnecessary backup/restore of GL_ELEMENT_ARRAY_BUFFER_BINDING since this is part of the VAO state.
//  2018-05-14: OpenGL: Making the call to glBindSampler() optional so 3.2 context won't fail if the function is a None pointer.
...
{
    return ImGui::GetCurrentContext() ? (ImGui_ImplOpenGL3_Data*)ImGui::GetIO().Backendrenderer_user_data : None;
}
...
bool    ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
    ...
    // Note: GLSL version is NOT the same as GL version. Leave this to None if unsure.
    ...
    for (GLint i = 0; i < num_extensions; i += 1)
    {
        ...
        if (extension != None && strcmp(extension, "GL_ARB_clip_control") == 0)
...
{
    ImGui_ImplOpenGL3_Data* bd = ImGui_ImplOpenGL3_GetBackendData();
    IM_ASSERT(bd != None && "Did you call ImGui_ImplOpenGL3_Init()?");
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n += 1)
    {
        ...
        if (bd->UseBufferSubData)
        {
            if (bd->VertexBufferSize < vtx_buffer_size)
            {
                ...
                glBufferData(GL_ARRAY_BUFFER, bd->VertexBufferSize, None, GL_STREAM_DRAW);
            }
            if (bd->IndexBufferSize < idx_buffer_size)
            {
                bd->IndexBufferSize = idx_buffer_size;
                glBufferData(GL_ELEMENT_ARRAY_BUFFER, bd->IndexBufferSize, None, GL_STREAM_DRAW);
            }
...
static bool CheckShader(GLuint handle, const char* desc)
{
    ...
    if (log_length > 1)
    {
        ...
        glGetShaderInfoLog(handle, log_length, None, (GLchar*)buf.begin());
        fprintf(stderr, "%s\n", buf.begin());
    }
...
static bool CheckProgram(GLuint handle, const char* desc)
{
    ...
    if (log_length > 1)
    {
        ImVector<char> buf;
        buf.resize((log_length + 1));
        glGetProgramInfoLog(handle, log_length, None, (GLchar*)buf.begin());
        fprintf(stderr, "%s\n", buf.begin());
    }
    return (GLboolean)status == GL_TRUE;
}
...
bool    ImGui_ImplOpenGL3_CreateDeviceObjects()
{
    ...
    const GLchar* vertex_shader = None;
    const GLchar* fragment_shader = None;
    if (glsl_version < 130)
    {
    ...
    glShaderSource(vert_handle, 2, vertex_shader_with_version, None);
    glCompileShader(vert_handle);
    CheckShader(vert_handle, "vertex shader");

    const GLchar* fragment_shader_with_version[2] = { bd->GlslVersionString, fragment_shader };
    GLuint frag_handle = glCreateShader(GL_FRAGMENT_SHADER);
    glShaderSource(frag_handle, 2, fragment_shader_with_version, None);
