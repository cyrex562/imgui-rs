...
//  2019-02-16: Vulkan: viewport and clipping rectangles correctly using draw_data->framebuffer_scale to allow retina display.
...
//  2018-06-08: Vulkan: Use draw_data->display_pos and draw_data->display_size to setup projection matrix and clipping rectangle.
...
// Backend data stored in io.backend_renderer_user_data to allow support for multiple Dear ImGui contexts
// It is STRONGLY preferred that you use docking branch with multi-viewports (== single Dear ImGui context + multiple windows) instead of multiple Dear ImGui contexts.
// FIXME: multi-context support is not tested and probably dysfunctional in this backend.
static ImGui_ImplVulkan_Data* ImGui_ImplVulkan_GetBackendData()
{
    return ImGui::GetCurrentContext() ? (ImGui_ImplVulkan_Data*)ImGui::GetIO().Backendrenderer_user_data : None;
}
...
static uint32_t ImGui_ImplVulkan_MemoryType(VkMemoryPropertyFlags properties, uint32_t type_bits)
{
    ...
    for (uint32_t i = 0; i < prop.memoryTypeCount; i += 1)
...
static void ImGui_ImplVulkan_SetupRenderState(ImDrawData* draw_data, VkPipeline pipeline, VkCommandBuffer command_buffer, ImGui_ImplVulkanH_FrameRenderBuffers* rb, int fb_width, int fb_height)
{
    ...
    // Our visible imgui space lies from draw_data->DisplayPps (top left) to draw_data->display_pos+data_data->display_size (bottom right). display_pos is (0,0) for single viewport apps.
    {
        float scale[2];
        scale[0] = 2.0 / draw_data->DisplaySize.x;
        scale[1] = 2.0 / draw_data->DisplaySize.y;
        float translate[2];
        translate[0] = -1.0 - draw_data->DisplayPos.x * scale[0];
        translate[1] = -1.0 - draw_data->DisplayPos.y * scale[1];
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline)
{
    ...
    ImGui_ImplVulkan_ViewportData* viewport_renderer_data = (ImGui_ImplVulkan_ViewportData*)draw_data->OwnerViewport->renderer_user_data;
    IM_ASSERT(viewport_renderer_data != None);
    ImGui_ImplVulkanH_WindowRenderBuffers* wrb = &viewport_renderer_data->RenderBuffers;
    if (wrb->FrameRenderBuffers == None)
    {
        wrb->Index = 0;
    ...
    if (draw_data->TotalVtxCount > 0)
    {
        ...
        ImDrawVert* vtx_dst = None;
        ImDrawIdx* idx_dst = None;
    ...
    Vector2D clip_off = draw_data->DisplayPos;         // (0,0) unless using multi-viewports
    Vector2D clip_scale = draw_data->FramebufferScale; // (1,1) unless using retina display which are often (2,2)
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n += 1)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list.cmd_buffer.Size; cmd_i += 1)
        {
            ...
            else
            {
                ...
                if (sizeof(ImTextureID) < sizeof)
                {
                    // We don't support texture switches if ImTextureID hasn't been redefined to be 64-bit. Do a flaky check that other textures haven't been used.
                    IM_ASSERT(pcmd->TextureId == (ImTextureID)bd->FontDescriptorSet);
                    desc_set[0] = bd->FontDescriptorSet;
                }
                vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, bd->PipelineLayout, 0, 1, desc_set, 0, None);
...
bool ImGui_ImplVulkan_CreateFontsTexture(VkCommandBuffer command_buffer)
{
    ...
    // Copy to Image:
    {
        ...
        vkCmdPipelineBarrier(command_buffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, None, 0, None, 1, copy_barrier);
        ...
        vkCmdPipelineBarrier(command_buffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, None, 0, None, 1, use_barrier);
    }
...
bool    ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info, VkRenderPass render_pass)
{
    ...
    // Our render function expect renderer_user_data to be storing the window render buffer we need (for the main viewport we won't use ->window)
    ImGuiViewport* main_viewport = ImGui::GetMainViewport();
    main_viewport->renderer_user_data = IM_NEW(ImGui_ImplVulkan_ViewportData)();
...
void ImGui_ImplVulkan_Shutdown()
{
    ...
    ImGuiViewport* main_viewport = ImGui::GetMainViewport();
    if (ImGui_ImplVulkan_ViewportData* vd = (ImGui_ImplVulkan_ViewportData*)main_viewport->renderer_user_data)
        IM_DELETE(vd);
...
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    IM_ASSERT(bd != None && "Did you call ImGui_ImplVulkan_Init()?");
    IM_UNUSED(bd);
}
...
VkDescriptorSet ImGui_ImplVulkan_AddTexture(VkSampler sampler, VkImageView image_view, VkImageLayout image_layout)
{
    ...
    // update the Descriptor Set:
    {
        ...
        vkUpdateDescriptorSets(v->Device, 1, write_desc, 0, None);
    }
...
{
    IM_ASSERT(g_FunctionsLoaded && "Need to call ImGui_ImplVulkan_LoadFunctions() if IMGUI_IMPL_VULKAN_NO_PROTOTYPES or VK_NO_PROTOTYPES are set!");
    IM_ASSERT(request_formats != None);
    ...
    vkGetPhysicalDeviceSurfaceFormatsKHR(physical_device, surface, &avail_count, None);
    ...
    else
    {
        ...
        for (int request_i = 0; request_i < request_formats_count; request_i += 1)
            for (uint32_t avail_i = 0; avail_i < avail_count; avail_i += 1)
...
{
    IM_ASSERT(g_FunctionsLoaded && "Need to call ImGui_ImplVulkan_LoadFunctions() if IMGUI_IMPL_VULKAN_NO_PROTOTYPES or VK_NO_PROTOTYPES are set!");
    IM_ASSERT(request_modes != None);
    IM_ASSERT(request_modes_count > 0);

    // Request a certain mode and confirm that it is available. If not use VK_PRESENT_MODE_FIFO_KHR which is mandatory
    uint32_t avail_count = 0;
    vkGetPhysicalDeviceSurfacePresentModesKHR(physical_device, surface, &avail_count, None);
    ...
    for (int request_i = 0; request_i < request_modes_count; request_i += 1)
        for (uint32_t avail_i = 0; avail_i < avail_count; avail_i += 1)
...
void ImGui_ImplVulkanH_CreateWindowCommandBuffers(VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, uint32_t queue_family, const VkAllocationCallbacks* allocator)
{
    ...
    VkResult err;
    for (uint32_t i = 0; i < wd->ImageCount; i += 1)
    {
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator, int w, int h, uint32_t min_image_count)
{
    ...
    // Create Swapchain
    {
        ...
        check_vk_result(err);
        err = vkGetSwapchainImagesKHR(device, wd->Swapchain, &wd->ImageCount, None);
        check_vk_result(err);
...
void ImGui_ImplVulkanH_DestroyWindowRenderBuffers(VkDevice device, ImGui_ImplVulkanH_WindowRenderBuffers* buffers, const VkAllocationCallbacks* allocator)
{
    for (uint32_t n = 0; n < buffers->Count; n += 1)
        ImGui_ImplVulkanH_DestroyFrameRenderBuffers(device, &buffers->FrameRenderBuffers[n], allocator);
    IM_FREE(buffers->FrameRenderBuffers);
    buffers->FrameRenderBuffers = None;
    buffers->Index = 0;
    buffers->Count = 0;
}
...
void ImGui_ImplVulkanH_DestroyAllViewportsRenderBuffers(VkDevice device, const VkAllocationCallbacks* allocator)
{
    ...
    for (int n = 0; n < platform_io.Viewports.Size; n += 1)
        if (ImGui_ImplVulkan_ViewportData* vd = (ImGui_ImplVulkan_ViewportData*)platform_io.Viewports[n]->renderer_user_data)
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport* viewport)
{
    ...
    // Create surface
    ImGuiPlatformIO& platform_io = ImGui::GetPlatformIO();
    VkResult err = (VkResult)platform_io.platform_create_vk_surface(viewport, v->Instance, (const void*)v->Allocator, (ImU64*)&wd->Surface);
    check_vk_result(err);
...
static void ImGui_ImplVulkan_DestroyWindow(ImGuiViewport* viewport)
{
    ...
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    if (ImGui_ImplVulkan_ViewportData* vd = (ImGui_ImplVulkan_ViewportData*)viewport->renderer_user_data)
    {
        ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
        if (vd->WindowOwned)
...
static void ImGui_ImplVulkan_SetWindowSize(ImGuiViewport* viewport, Vector2D size)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_ViewportData* vd = (ImGui_ImplVulkan_ViewportData*)viewport->renderer_user_data;
    if (vd == None) // This is None for the main viewport (which is left to the user/app to handle)
        return;
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
...
static void ImGui_ImplVulkan_RenderWindow(ImGuiViewport* viewport, void*)
{
    ...
    ImGui_ImplVulkanH_FrameSemaphores* fsd = &wd->FrameSemaphores[wd->SemaphoreIndex];
    {
        ...
        }
        {
            ...
            info.pClearValues = (viewport->Flags & ImGuiViewportFlags_NoRendererClear) ? None : &wd->ClearValue;
...
void ImGui_ImplVulkan_InitPlatformInterface()
{
    ...
    if (ImGui::GetIO().ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
        IM_ASSERT(platform_io.platform_create_vk_surface != None && "Platform needs to setup the CreateVkSurface handler.");
    platform_io.Renderer_CreateWindow = ImGui_ImplVulkan_CreateWindow;
    platform_io.renderer_destroy_window = ImGui_ImplVulkan_DestroyWindow;
    platform_io.Renderer_SetWindowSize = ImGui_ImplVulkan_SetWindowSize;
    platform_io.renderer_render_window = ImGui_ImplVulkan_RenderWindow;
    platform_io.renderer_swap_buffers = ImGui_ImplVulkan_SwapBuffers;
}
