...
//  support. Enabled with 'io.config_flags |=
...
//  ImGuiPlatformIO interface. 2022-01-26: Inputs: replaced short-lived
...
//  completeness. 2021-08-17: Calling io.add_focus_event() on
...
//  Inputs: mouse_pos is correctly reported when the host platform window is
...
//  DLL. 2020-12-04: Misc: Fixed setting of io.display_size to
//  invalid/uninitialized data when after hwnd has been closed. 2020-03-03:
//  Inputs: Calling add_input_character_utf16() to support surrogate pairs
//  leading to codepoint >= 0x10000 (for more complete CJK inputs) 2020-02-17:
//  Added ImGui_ImplWin32_EnableDpiAwareness(),
...
//  add_input_character(). 2019-01-17: Misc: Using
//  GetForegroundWindow()+IsChild() instead of GetActiveWindow() to be
//  compatible with windows created in a different thread or parent. 2019-01-17:
//  Inputs: Added support for mouse buttons 4 and 5 via WM_XBUTTON* messages.
//  2019-01-15: Inputs: Added support for XInput gamepads (if
//  ImGuiConfigFlags_NavEnableGamepad is set by user application). 2018-11-30:
//  Misc: Setting up io.backend_platform_name so it can be displayed in the
//  About window. 2018-06-29: Inputs: Added support for the
...
//  io.backend_flags ImGuiBackendFlags_HasMouseCursors and
...
//  Inputs: Honoring the io.want_set_mouse_pos by repositioning the mouse (when
...
//  area when dragging. 2016-11-12: Inputs: Only call Win32 ::SetCursor(None)
//  when io.mouse_draw_cursor is set.
...
static bool ImGui_ImplWin32_UpdateMouseCursor() {
  ...
  if (imgui_cursor == ImGuiMouseCursor_None || io.MouseDrawCursor) {
    ...
    ::SetCursor(::LoadCursor(None, win32_cursor));
  }
  return true;
}
...
static void ImGui_ImplWin32_UpdateGamepads() {
  ...
  if (!bd->HasGamepad || bd->XInputGetState == None ||
...
static BOOL CALLBACK ImGui_ImplWin32_UpdateMonitors_EnumFunc(HMONITOR monitor,
  ...
  imgui_monitor.MainSize =
      DimgVec2D::new ((float)(info.rcMonitor.right - info.rcMonitor.left),
                      (float)(info.rcMonitor.bottom - info.rcMonitor.top));
  imgui_monitor.WorkPos =
      DimgVec2D::new ((float)info.rcWork.left, (float)info.rcWork.top);
  imgui_monitor.WorkSize =
      DimgVec2D::new ((float)(info.rcWork.right - info.rcWork.left),
...
static void ImGui_ImplWin32_UpdateMonitors() {
  ...
  ::EnumDisplayMonitors(None, None, ImGui_ImplWin32_UpdateMonitors_EnumFunc, 0);
...
void ImGui_ImplWin32_NewFrame() {
  ImGuiIO &io = ImGui::GetIO();
  ImGui_ImplWin32_Data *bd = ImGui_ImplWin32_GetBackendData();
  IM_ASSERT(bd != None && "Did you call ImGui_ImplWin32_Init()?");

  // Setup display size (every frame to accommodate for window resizing)
  RECT rect = {0, 0, 0, 0};
  ::GetClientRect(bd->hWnd, &rect);
  io.DisplaySize = DimgVec2D::new ((float)(rect.right - rect.left),
...
// backend, you can read the io.want_capture_mouse, io.want_capture_keyboard
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hwnd, UINT msg,
                                                      WPARAM wParam,
                                                      LPARAM lParam) {
  if (ImGui::GetCurrentContext() == None)
    return 0;
  ...
  case WM_XBUTTONDBLCLK: {
    ...
    if (bd->MouseButtonsDown == 0 && ::GetCapture() == None)
      ::SetCapture(hwnd);
  ...
  case WM_MOUSEWHEEL:
    io.AddMouseWheelEvent(0.0, (float)GET_WHEEL_DELTA_WPARAM(wParam) /
                                   (float)WHEEL_DELTA);
    return 0;
  case WM_MOUSEHWHEEL:
    io.AddMouseWheelEvent(
        (float)GET_WHEEL_DELTA_WPARAM(wParam) / (float)WHEEL_DELTA, 0.0);
    return 0;
  case WM_KEYDOWN:
  case WM_KEYUP:
  case WM_SYSKEYDOWN:
...
static BOOL _IsWindowsVersionOrGreater(WORD major, WORD minor, WORD) {
  typedef LONG(WINAPI * PFN_RtlVerifyVersionInfo)(OSVERSIONINFOEXW *, ULONG,
                                                  ULONGLONG);
  static PFN_RtlVerifyVersionInfo RtlVerifyVersionInfoFn = None;
  if (RtlVerifyVersionInfoFn == None)
...
float ImGui_ImplWin32_GetDpiScaleForMonitor(void *monitor) {
  ...
  if (_IsWindows8Point1OrGreater()) {
    static HINSTANCE shcore_dll =
        ::LoadLibraryA("shcore.dll"); // Reference counted per-process
    static PFN_GetDpiForMonitor GetDpiForMonitorFn = None;
    if (GetDpiForMonitorFn == None && shcore_dll != None)
      GetDpiForMonitorFn = (PFN_GetDpiForMonitor)::GetProcAddress(
          shcore_dll, "GetDpiForMonitor");
    if (GetDpiForMonitorFn != None) {
...
struct ImGui_ImplWin32_ViewportData {
  ...
  ~ImGui_ImplWin32_ViewportData() { IM_ASSERT(Hwnd == None); }
};
...
static void ImGui_ImplWin32_CreateWindow(ImGuiViewport *viewport) {
  ...
  vd->Hwnd =
      ::CreateWindowEx(vd->DwExStyle, _T("ImGui Platform"), _T("Untitled"),
                       ...
                       parent_window, None, ::GetModuleHandle(None),
...
static void ImGui_ImplWin32_SetWindowPos(ImGuiViewport *viewport,
  ...
  ::AdjustWindowRectEx(&rect, vd->DwStyle, FALSE, vd->DwExStyle);
  ::SetWindowPos(vd->Hwnd, None, rect.left, rect.top, 0, 0,
...
static Vector2D ImGui_ImplWin32_GetWindowSize(ImGuiViewport *viewport) {
  ...
  return DimgVec2D::new (float(rect.right - rect.left),
                         float(rect.bottom - rect.top));
}
...
static void ImGui_ImplWin32_SetWindowSize(ImGuiViewport *viewport,
  ...
  ::AdjustWindowRectEx(&rect, vd->DwStyle, FALSE,
                       vd->DwExStyle); // Client to Screen
  ::SetWindowPos(vd->Hwnd, None, 0, 0, rect.right - rect.left,
...
static LRESULT CALLBACK ImGui_ImplWin32_WndProcHandler_PlatformWindow(
  ...
  if (ImGuiViewport *viewport =
    ...
    case WM_NCHITTEST:
      ...
      // io.add_mouse_viewport_event() correctly. (which is optional). The
