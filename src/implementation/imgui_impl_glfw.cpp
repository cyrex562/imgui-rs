...
//  [x] Platform: Keyboard support. Since 1.87 we are using the
//  io.add_key_event() function. Pass ImGuiKey values to all key functions e.g.
...
//  support. Enable with 'io.config_flags |= ImGuiConfigFlags_NavEnableGamepad'.
//  [x] Platform: Mouse cursor shape and visibility. Disable with
//  'io.config_flags |= ImGuiConfigFlags_NoMouseCursorChange' (note: the
//  resizing cursors requires GLFW 3.4+). [x] Platform: Multi-viewport support
//  (multiple windows). Enable with 'io.config_flags |=
//  ImGuiConfigFlags_ViewportsEnable'.

// Issues:
//  [ ] Platform: Multi-viewport support: ParentViewportID not honored, and so
//  io.config_viewports_no_default_parent has no effect (minor).
...
//  ImGuiPlatformIO interface. 2022-04-30: Inputs: Fixed
...
//  weeks ago)with io.add_key_event() using ImGuiKey_ModXXX flags. Sorry for the
//  confusion. 2021-01-20: Inputs: calling new io.add_key_analog_event() for
//  gamepad support, instead of writing directly to io.nav_inputs[]. 2022-01-17:
//  Inputs: calling new io.add_mouse_pos_event(), io.add_mouse_button_event(),
...
//  2022-01-10: Inputs: calling new io.add_key_event(), io.AddKeyModsEvent() +
//  io.set_key_event_native_data() API (1.87+). Support for full ImGuiKey range.
...
//  glfwSetWindowFocusCallback() to calling io.add_focus_event(). If you called
...
//  using glfwSetCursorEnterCallback(). mouse_pos is correctly reported when the
...
//  calling add_input_character(). 2019-03-12: Misc: Preserve
//  display_framebuffer_scale when main window is minimized. 2018-11-30: Misc:
//  Setting up io.backend_platform_name so it can be displayed in the About
...
//  io.backend_flags ImGuiBackendFlags_HasMouseCursors flag + honor
...
//  io.want_set_mouse_pos by repositioning the mouse (when using navigation and
...
//  2017-08-25: Inputs: mouse_pos set to -FLT_MAX,-FLT_MAX when mouse is
...
#define GLFW_HAS_FOCUS_WINDOW                                                  \
  (GLFW_VERSION_MAJOR * 1000 + GLFW_VERSION_MINOR * 100 >=                     \
   3200) // 3.2+ glfwFocusWindow
#define GLFW_HAS_FOCUS_ON_SHOW                                                 \
  (GLFW_VERSION_MAJOR * 1000 + GLFW_VERSION_MINOR * 100 >=                     \
   3300) // 3.3+ GLFW_FOCUS_ON_SHOW
#define GLFW_HAS_MONITOR_WORK_AREA                                             \
  (GLFW_VERSION_MAJOR * 1000 + GLFW_VERSION_MINOR * 100 >=                     \
   3300) // 3.3+ glfwGetMonitorWorkarea
...
#define GLFW_HAS_MOUSE_PASSTHROUGH                                             \
  (GLFW_VERSION_MAJOR * 1000 + GLFW_VERSION_MINOR * 100 >=                     \
   3400) // 3.4+ GLFW_MOUSE_PASSTHROUGH
#else
...
// Backend data stored in io.backend_platform_user_data to allow support for
...
static ImGui_ImplGlfw_Data *ImGui_ImplGlfw_GetBackendData() {
  return ImGui::GetCurrentContext()
             ? (ImGui_ImplGlfw_Data *)ImGui::GetIO().Backendplatform_user_data
             : None;
}
...
void ImGui_ImplGlfw_MouseButtonCallback(GLFWwindow *window, int button,
  ...
  if (bd->PrevUserCallbackMousebutton != None && window == bd->Window)
...
void ImGui_ImplGlfw_ScrollCallback(GLFWwindow *window, double xoffset,
                                   double yoffset) {
  ImGui_ImplGlfw_Data *bd = ImGui_ImplGlfw_GetBackendData();
  if (bd->PrevUserCallbackScroll != None && window == bd->Window)
...
void ImGui_ImplGlfw_KeyCallback(GLFWwindow *window, int keycode, int scancode,
                                int action, int mods) {
  ImGui_ImplGlfw_Data *bd = ImGui_ImplGlfw_GetBackendData();
  if (bd->PrevUserCallbackKey != None && window == bd->Window)
  ...
  if (keycode >= 0 && keycode < IM_ARRAYSIZE(bd->KeyOwnerWindows))
    bd->KeyOwnerWindows[keycode] = (action == GLFW_PRESS) ? window : None;
...
void ImGui_ImplGlfw_WindowFocusCallback(GLFWwindow *window, int focused) {
  ImGui_ImplGlfw_Data *bd = ImGui_ImplGlfw_GetBackendData();
  if (bd->PrevUserCallbackWindowFocus != None && window == bd->Window)
...
void ImGui_ImplGlfw_CursorPosCallback(GLFWwindow *window, double x, double y) {
  ImGui_ImplGlfw_Data *bd = ImGui_ImplGlfw_GetBackendData();
  if (bd->PrevUserCallbackCursorPos != None && window == bd->Window)
  ...
  bd->LastValidMousePos = DimgVec2D::new ((float)x, (float)y);
}
...
void ImGui_ImplGlfw_CursorEnterCallback(GLFWwindow *window, int entered) {
  ImGui_ImplGlfw_Data *bd = ImGui_ImplGlfw_GetBackendData();
  if (bd->PrevUserCallbackCursorEnter != None && window == bd->Window)
...
void ImGui_ImplGlfw_CharCallback(GLFWwindow *window, unsigned int c) {
  ImGui_ImplGlfw_Data *bd = ImGui_ImplGlfw_GetBackendData();
  if (bd->PrevUserCallbackChar != None && window == bd->Window)
...
void ImGui_ImplGlfw_RestoreCallbacks(GLFWwindow *window) {
  ...
  bd->PrevUserCallbackWindowFocus = None;
  bd->PrevUserCallbackCursorEnter = None;
  bd->PrevUserCallbackCursorPos = None;
  bd->PrevUserCallbackMousebutton = None;
  bd->PrevUserCallbackScroll = None;
  bd->PrevUserCallbackKey = None;
  bd->PrevUserCallbackChar = None;
  bd->PrevUserCallbackMonitor = None;
}
...
static bool ImGui_ImplGlfw_Init(GLFWwindow *window, bool install_callbacks,
  ...
  io.BackendFlags |=
      ImGuiBackendFlags_HasMouseCursors; // We can honor GetMouseCursor() values
                                         // (optional)
  io.BackendFlags |=
      ImGuiBackendFlags_HasSetMousePos; // We can honor io.want_set_mouse_pos
  ...
  // Missing cursors will return None and our _UpdateMouseCursor() function will
  // use the Arrow cursor instead.)
  GLFWerrorfun prev_error_callback = glfwSetErrorCallback(None);
  ...
  main_viewport->platform_handle = (void *)bd->Window;
#ifdef _WIN32
  main_viewport->platform_handleRaw = glfwGetWin32Window(bd->window);
#elif defined(__APPLE__)
  main_viewport->platform_handleRaw = (void *)glfwGetCocoaWindow(bd->window);
#endif
  if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
...
static void ImGui_ImplGlfw_UpdateMouseData() {
  ...
  for (int n = 0; n < platform_io.Viewports.Size; n += 1) {
    ...
    GLFWwindow *window = (GLFWwindow *)viewport->platform_handle;
    ...
    if (is_window_focused) {
      ...
      // captured)
      if (bd->MouseWindow == None) {
        double mouse_x, mouse_y;
        glfwGetCursorPos(window, &mouse_x, &mouse_y);
        if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable) {
          // Single viewport mode: mouse position in client window coordinates
          // (io.mouse_pos is (0,0) when the mouse is on the upper-left corner
          // of the app window) Multi-viewport mode: mouse position in OS
          // absolute coordinates (io.mouse_pos is (0,0) when the mouse is on
        ...
        bd->LastValidMousePos = DimgVec2D::new ((float)mouse_x, (float)mouse_y);
        io.AddMousePosEvent((float)mouse_x, (float)mouse_y);
      }
    }

    // (Optional) When using multiple viewports: call
    // io.add_mouse_viewport_event() with the viewport the OS mouse cursor is
    // hovering. If ImGuiBackendFlags_HasMouseHoveredViewport is not set by the
    // backend, Dear imGui will ignore this field and infer the information
...
static void ImGui_ImplGlfw_UpdateMouseCursor() {
  ...
  ImGuiPlatformIO &platform_io = ImGui::GetPlatformIO();
  for (int n = 0; n < platform_io.Viewports.Size; n += 1) {
    GLFWwindow *window = (GLFWwindow *)platform_io.Viewports[n]->platform_handle;
...
#define MAP_BUTTON(KEY_NO, BUTTON_NO, _UNUSED)                                 \
  do {                                                                         \
    io.add_key_event(KEY_NO, gamepad.buttons[BUTTON_NO] != 0);                 \
  } while (0)
#define MAP_ANALOG(KEY_NO, AXIS_NO, _UNUSED, V0, V1)                           \
  do {                                                                         \
    float v = gamepad.axes[AXIS_NO];                                           \
    v = (v - V0) / (V1 - V0);                                                  \
    io.add_key_analog_event(KEY_NO, v > 0.10, Saturate(v));                    \
  } while (0)
#else
...
#define MAP_ANALOG(KEY_NO, _UNUSED, AXIS_NO, V0, V1)                           \
  do {                                                                         \
    ...
    v = (v - V0) / (V1 - V0);                                                  \
    io.AddKeyAnalogEvent(KEY_NO, v > 0.10, Saturate(v));                       \
  } while (0)
#endif
  io.BackendFlags |= ImGuiBackendFlags_HasGamepad;
  ...
  MAP_ANALOG(ImGuiKey_GamepadL2, GLFW_GAMEPAD_AXIS_LEFT_TRIGGER, 4, -0.75,
             +1.0);
  MAP_ANALOG(ImGuiKey_GamepadR2, GLFW_GAMEPAD_AXIS_RIGHT_TRIGGER, 5, -0.75,
             +1.0);
  MAP_BUTTON(ImGuiKey_GamepadL3, GLFW_GAMEPAD_BUTTON_LEFT_THUMB, 8);
  MAP_BUTTON(ImGuiKey_GamepadR3, GLFW_GAMEPAD_BUTTON_RIGHT_THUMB, 9);
  MAP_ANALOG(ImGuiKey_GamepadLStickLeft, GLFW_GAMEPAD_AXIS_LEFT_X, 0, -0.25,
             -1.0);
  MAP_ANALOG(ImGuiKey_GamepadLStickRight, GLFW_GAMEPAD_AXIS_LEFT_X, 0, +0.25,
             +1.0);
  MAP_ANALOG(ImGuiKey_GamepadLStickUp, GLFW_GAMEPAD_AXIS_LEFT_Y, 1, -0.25,
             -1.0);
  MAP_ANALOG(ImGuiKey_GamepadLStickDown, GLFW_GAMEPAD_AXIS_LEFT_Y, 1, +0.25,
             +1.0);
  MAP_ANALOG(ImGuiKey_GamepadRStickLeft, GLFW_GAMEPAD_AXIS_RIGHT_X, 2, -0.25,
             -1.0);
  MAP_ANALOG(ImGuiKey_GamepadRStickRight, GLFW_GAMEPAD_AXIS_RIGHT_X, 2, +0.25,
             +1.0);
  MAP_ANALOG(ImGuiKey_GamepadRStickUp, GLFW_GAMEPAD_AXIS_RIGHT_Y, 3, -0.25,
             -1.0);
  MAP_ANALOG(ImGuiKey_GamepadRStickDown, GLFW_GAMEPAD_AXIS_RIGHT_Y, 3, +0.25,
             +1.0);
#undef MAP_BUTTON
#undef MAP_ANALOG
}
...
static void ImGui_ImplGlfw_UpdateMonitors() {
  ...
  for (int n = 0; n < monitors_count; n += 1) {
    ...
    monitor.MainPos = monitor.WorkPos = DimgVec2D::new ((float)x, (float)y);
    monitor.MainSize = monitor.WorkSize =
        DimgVec2D::new ((float)vid_mode->width, (float)vid_mode->height);
    ...
    {
      monitor.WorkPos = DimgVec2D::new ((float)x, (float)y);
      monitor.WorkSize = DimgVec2D::new ((float)w, (float)h);
    }
#endif
...
void ImGui_ImplGlfw_NewFrame() {
  ImGuiIO &io = ImGui::GetIO();
  ImGui_ImplGlfw_Data *bd = ImGui_ImplGlfw_GetBackendData();
  IM_ASSERT(bd != None && "Did you call ImGui_ImplGlfw_InitForXXX()?");
  ...
  if (w > 0 && h > 0)
    io.DisplayFramebufferScale = DimgVec2D::new ((float)display_w / (float)w,
  ...
  io.DeltaTime =
      bd->Time > 0.0 ? (float)(current_time - bd->Time) : (float)(1.0 / 60.0);
...
struct ImGui_ImplGlfw_ViewportData {
  ...
  ~ImGui_ImplGlfw_ViewportData() { IM_ASSERT(Window == None); }
};

static void ImGui_ImplGlfw_WindowCloseCallback(GLFWwindow *window) {
  if (ImGuiViewport *viewport = ImGui::FindViewportByplatform_handle(window))
...
static void ImGui_ImplGlfw_WindowPosCallback(GLFWwindow *window, int, int) {
  if (ImGuiViewport *viewport = ImGui::FindViewportByplatform_handle(window)) {
    if (ImGui_ImplGlfw_ViewportData *vd =
            (ImGui_ImplGlfw_ViewportData *)viewport->platform_user_data) {
      bool ignore_event =
...
static void ImGui_ImplGlfw_CreateWindow(ImGuiViewport *viewport) {
  ...
  GLFWwindow *share_window =
      (bd->ClientApi == GlfwClientApi_OpenGL) ? bd->Window : None;
  vd->Window = glfwCreateWindow(viewport->Size.x, viewport->Size.y,
                                "No Title Yet", None, share_window);
  vd->WindowOwned = true;
  viewport->platform_handle = (void *)vd->Window;
#ifdef _WIN32
  viewport->platform_handleRaw = glfwGetWin32Window(vd->window);
#elif defined(__APPLE__)
  viewport->platform_handleRaw = (void *)glfwGetCocoaWindow(vd->window);
#endif
...
// Linux/OSX!)
#if !GLFW_HAS_MOUSE_PASSTHROUGH && GLFW_HAS_WINDOW_HOVERED && defined(_WIN32)
static WNDPROC g_GlfwWndProc = None;
static LRESULT CALLBACK WndProcNoInputs(HWND hWnd, UINT msg, WPARAM wParam,
  ...
  if (msg == WM_NCHITTEST) {
    ...
    // io.add_mouse_viewport_event() properly (which is OPTIONAL). The
...
static void ImGui_ImplGlfw_SetWindowSize(ImGuiViewport *viewport,
                                         Vector2D size) {
  ImGui_ImplGlfw_ViewportData *vd =
      (ImGui_ImplGlfw_ViewportData *)viewport->platform_user_data;
...
static void ImGui_ImplGlfw_InitPlatformInterface() {
  ...
  platform_io.platform_destroy_window = ImGui_ImplGlfw_DestroyWindow;
  platform_io.Platform_ShowWindow = ImGui_ImplGlfw_ShowWindow;
  platform_io.Platform_SetWindowPos = ImGui_ImplGlfw_SetWindowPos;
  platform_io.platform_get_window_pos = ImGui_ImplGlfw_GetWindowPos;
  platform_io.Platform_SetWindowSize = ImGui_ImplGlfw_SetWindowSize;
  platform_io.platform_get_window_size = ImGui_ImplGlfw_GetWindowSize;
  platform_io.platform_set_window_focus = ImGui_ImplGlfw_SetWindowFocus;
  platform_io.platform_get_window_focus = ImGui_ImplGlfw_GetWindowFocus;
  platform_io.platform_get_window_minimized = ImGui_ImplGlfw_GetWindowMinimized;
  platform_io.Platform_SetWindowTitle = ImGui_ImplGlfw_SetWindowTitle;
  platform_io.platform_render_window = ImGui_ImplGlfw_RenderWindow;
  platform_io.platform_swap_buffers = ImGui_ImplGlfw_SwapBuffers;
#if GLFW_HAS_WINDOW_ALPHA
  platform_io.Platform_SetWindowAlpha = ImGui_ImplGlfw_SetWindowAlpha;
#endif
#if GLFW_HAS_VULKAN
  platform_io.platform_create_vk_surface = ImGui_ImplGlfw_CreateVkSurface;
#endif
